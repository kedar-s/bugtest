...
public class LoadRules extends RESTfulClient {
	...
	public boolean uploadFile(File ruleFile) throws Exception {
        try {
            ...
            ClientResponse response = webResource.queryParam("packageBodyName", ruleFile.getName()).cookie(getTokenCookie()).type(MediaType.APPLICATION_XML).entity(xml).post(ClientResponse.class);
