...
public class AccountFilter extends AuthorizationFilter {
    ...
    private static Logger logger = Logger.getLogger(AccountFilter.class);
    ...
    protected boolean isAccessAllowed(ServletRequest servletRequest, ServletResponse servletResponse, Object chain) throws Exception {
        ...
        if (userContext == null) {
            //The vestibule must be entered at least once before an account URL can be processed
            //TODO This should be configurable, based on userContext
            throw new TolvenAuthenticationException("An account must be selected", principal, sessionWrapper.getRealm());
            ...
            if (accountUser == null) {
                throw new TolvenAuthorizationException("Could not find accountUser " + accountUserIdString, principal, sessionWrapper.getRealm());
            }
            TolvenUser user = getActivationBean().findUser(principal);
            if (user == null) {
                throw new TolvenAuthorizationException("No TolvenUser found", principal, sessionWrapper.getRealm());
            }
            if (accountUser.getUser().getId() != user.getId()) {
                throw new TolvenAuthorizationException("Tolven User Id: " + user.getId() + " not authorized to use account: " + accountUser.getAccount().getId(), principal, sessionWrapper.getRealm());
            }
