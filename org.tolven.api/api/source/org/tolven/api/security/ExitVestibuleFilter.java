    ...
    protected boolean isAccessAllowed(ServletRequest servletRequest, ServletResponse servletResponse, Object chain) throws Exception {
        ...
        if (userContext == null) {
            ...
            if (proposedAccountUserIdString == null) {
                throw new TolvenAuthenticationException("Cannot exit vestibule without an account selected", principal, sessionWrapper.getRealm());
            } else {
                ...
                if (accountUser == null) {
                    throw new TolvenAuthorizationException("accountUser does not exist with Id: " + accountUserId, principal, sessionWrapper.getRealm());
                }
                ...
                if (privateKey == null) {
                    throw new TolvenAuthorizationException("User requires a UserPrivateKey to log into account: " + accountUser.getAccount().getId(), principal, sessionWrapper.getRealm());
                }
                ...
                if (accountUser.getUser().getId() != user.getId()) {
                    throw new TolvenAuthorizationException("User does not belong to account: " + accountUser.getId(), principal, sessionWrapper.getRealm());
                }
                ...
                logger.info(principal + " in realm: " + realm + " EXIT_VESTIBULE_TO_ACCOUNT: " + accountUser.getAccount().getId());
            }
        } else {
