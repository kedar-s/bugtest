 ...
 *
 * @author Joseph Isaac
 * @version $Id: AccountResources.java 6697 2012-06-26 02:57:46Z joe.isaac $
 */  
...
public class AccountResources {
    ...
    */
    ...
    public Response createAccount(
        ...
        try {
            MenuStructure rootMS = menuBean.getRootMenuStructure(accountType);
            Account account = null;
            if (Boolean.parseBoolean(template)) {
                account = accountBean.createAccount(accountType);
                rootMS.setAccount(account);
            } else {
                ...
                Account templateAccount = accountBean.findAccountTemplate(accountType.getKnownType());
                ...
                AccountUser accountUser = accountBean.addAccountUser(account, user, timeNow, true, userPublicKey);
    ...
    */
    ...
    public Response setAccountProperty(
        ...
        if (pname == null || pname.trim().length() == 0) {
            return Response.status(Status.BAD_REQUEST).type(MediaType.TEXT_PLAIN).entity("Property Name is missing").build();
        }
        Long accountUserId = (Long) request.getAttribute(GeneralSecurityFilter.ACCOUNTUSER_ID);
        if (accountUserId == null) {
            return Response.status(Status.FORBIDDEN).type(MediaType.TEXT_PLAIN).entity("Account required").build();
