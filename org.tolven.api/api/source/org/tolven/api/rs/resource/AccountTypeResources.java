 ...
 *
 * @author Joseph Isaac
 * @version $Id: AccountTypeResources.java 6180 2012-03-29 10:25:16Z joe.isaac $
 */
...
public class AccountTypeResources {
    ...
    public Response getAccountTypeProperty(
            @QueryParam("tpname") String tpname) {
        if (tpname == null || tpname.trim().length() == 0) {
            return Response.status(Status.BAD_REQUEST).type(MediaType.TEXT_PLAIN).entity("Account Type Property Name is missing").build();
        }
        ...
        try {
        	String tpvalue = templateAccount.getProperty().get(tpname);
        	
        	/*if(pvalue.startsWith("<?xml")) {
        		return XSLTUtil.generateXSLTResponse(pvalue, accountUser);
	        	
        	} else {*/
	        	Response response = Response.ok().entity(tpvalue).build(); 
	        	return response;
        	//}
        } catch (Exception ex) {
            return Response.status(Status.INTERNAL_SERVER_ERROR).type(MediaType.TEXT_PLAIN).entity(ExceptionFormatter.toSimpleString(ex, "\\n")).build();
    ...
    //@Produces
    @Produces(MediaType.TEXT_PLAIN)
    public Response setAccountTypeProperty(
            @FormParam("tpname") String tpname,		
            @FormParam("tpvalue") String tpvalue) {
        if (tpname == null || tpname.trim().length() == 0) {
            return Response.status(Status.BAD_REQUEST).type(MediaType.TEXT_PLAIN).entity("Account Type Property Name is missing").build();
        }
        Long accountUserId = (Long) request.getAttribute(GeneralSecurityFilter.ACCOUNTUSER_ID);
        ...
        Account templateAccount = accountUser.getAccount().getAccountTemplate();        
        try {
        	getAccountBean().putAccountProperty(templateAccount.getId(), tpname, tpvalue);
