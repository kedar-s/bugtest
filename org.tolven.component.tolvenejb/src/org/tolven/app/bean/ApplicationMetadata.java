...
//		}
		...
		if (fsb.length() > 0) {
			if(msColumn.getFrom() != null && msColumn.getFrom().length() > 0){
				fsb.append('|');
				fsb.append(msColumn.getFrom());
			}
...
//		nominateDefaultSuffix( ms );
	...
	public void processTrimMenus( java.util.List<Placeholder> trimMenus ) {
		java.util.List<String> menuNames = new ArrayList<String>();
		for(Placeholder placeholder:trimMenus){
			menuNames.add(placeholder.getName());
		}
		if(!menuNames.isEmpty())
			deleteColumnsForTrimMenus(menuNames);
		for (Placeholder placeholder : trimMenus) {
	...
	public void deleteColumnsForTrimMenus(java.util.List<String> menuNames){
		logger.debug("deleting MSColumn.fromDocument for trimmenus");
		 Query query = em.createQuery("delete from MSColumn mc WHERE mc.menuStructure in (select ml.menuStructure from MenuLocator ml where ml.path in (:menuNames))");
         query.setParameter("menuNames", menuNames);
         query.executeUpdate();
	}
...
//          childPlaceholder.setSequence(-1);
	 ...
	 * @param trimMenus
	 */
	public void processExtends( java.util.List<Application> extendApplications, java.util.List<Placeholder> trimMenus ) {
        ...
        for (Application application : extendApplications) {
    		...
    		trimMenus.addAll( application.getTrimMenus() );
	 ...
	 * @param trimMenus 
	 */
    public void processNewApplication(Application app, java.util.List<Placeholder> trimMenus) {
        ...
        trimMenus.addAll(app.getTrimMenus());
        if (app.getMenu() == null) {
            // Create default root
	 ...
	 */
	public void processApplications( java.util.List<Application> newApplications,java.util.List<Placeholder> trimMenus ) {
        ...
        for (Application application : newApplications) {
        	processNewApplication(application,trimMenus);
        }
	...
	public void loadApplications( Map<String, String> appFiles ) {
		...
		java.util.List<Placeholder> trimMenus = new ArrayList<Placeholder>();
		processApplications( newApplications,trimMenus );
        // Extensions
        processExtends( extendApplications,trimMenus );
        processTrimMenus(trimMenus);
