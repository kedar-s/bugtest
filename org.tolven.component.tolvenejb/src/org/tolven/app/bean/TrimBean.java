...
import org.tolven.trim.PlaceholderField;
...
@Stateless
public class TrimBean implements TrimLocal {
	...
	public TrimEx parseTrim( Reader input, TrimExpressionEvaluator ee ) throws JAXBException {
		TrimEx trim = (TrimEx) getTrimMarshaller().unmarshal(TRIMns, input);
		String extend = trim.getExtends();
		if (extend!=null) {
			TrimHeader trimHeader = this.findExtendedTrimHeader(extend,true);
	...
	public TrimEx parseTrim( String trimXML, TrimExpressionEvaluator ee ) throws JAXBException {
		...
		//CCHIT merge
        String trimStr=evaluatedTrimXML;
        trimXML="";
        ...
        while(trimStr.length()>0) {
                //trim=xml.substring(beginIndex)
                endIndex=trimStr.indexOf("<displayName>");
                startTag="<displayName>";
                endTag="</displayName>";
                
                if(endIndex!=-1) {
                        body=escapeXML(trimStr.substring(trimStr.indexOf(startTag)+startTag.length(),trimStr.indexOf(endTag)));
                        trimXML=trimXML+trimStr.substring(startIndex,endIndex)+startTag+body+endTag;                
                        trimStr=trimStr.substring(trimStr.indexOf(endTag)+endTag.length());
                }else {
                        break;
                }
                //startIndex=endIndex;
...
//		String expandedTrim = (String) ee.evaluate(new String( trim ));
	...
	//to find the extended trim. if it's not activated yet activate it now
	public TrimHeader findExtendedTrimHeader( String name,boolean activateNew ) {
		TrimHeader trimHeader = findOptionalTrimHeader( name);
		if (trimHeader==null) {
			trimHeader = findNewTrimHeader(name);			
		}
		if (activateNew && trimHeader==null) {
			trimHeader = findNewTrimHeader(name);
			if(trimHeader != null){
	...
	public TrimHeader findNewTrimHeader( String name ) {
		try {
			Query query = em.createQuery("SELECT t FROM TrimHeader t WHERE t.name = :name and t.status = :status");
			query.setParameter("name", name );
			query.setParameter("status", Status.NEW.value());
			return (TrimHeader) query.getSingleResult();
		} catch (NoResultException e ) {
			return null;
		}
...
//				continue;
	...
	public void addTrimToMenus( TrimHeader trimHeader ) throws JAXBException {
		...
		//Set the path of the trim header as specified in the transitions placeholder tag
		if(trim.getTransitions() != null && trim.getTransitions().getPlaceholder() != null 	&& trim.getTransitions().getPlaceholder().getPath() != null
				&& (trim.isAbstract() == null || (trim.isAbstract() != null && !trim.isAbstract()))) {
			trimHeader.setPath(trim.getTransitions().getPlaceholder().getPath());
			//Set the code of the trim header as specified in the transitions placeholder tag path 
			CD codeCD = null;
			//TODO: add exception handling that requires a Field if you have a Placeholder
			if (trim.getTransitions().getPlaceholder().getCodeField() != null) {
				PlaceholderField pf = trim.getTransitions().getPlaceholder().getCodeField();
				String path2value = pf.getPath();
				//EE to tranverse the trim down this path and get the value living there
				TrimExpressionEvaluator evaluator = new TrimExpressionEvaluator();
				evaluator.addVariable("trim", trim);			
				String path = "#{" + path2value + "}";
				codeCD = (CD) evaluator.evaluate(path);
				if (codeCD != null) {
					trimHeader.setCode(codeCD.getCode());
					trimHeader.setCodeSystem(codeCD.getCodeSystem());
					trimHeader.setCodeSystemName(codeCD.getCodeSystemName());		
				} else {
					throw new RuntimeException("TrimBean:addTrimToMenus: Act, Entity or Role with bind statement requires code element");
				}
			}
			
			for(Transition transition : trim.getTransitions().getTransitions()) {
				PlaceholderTransition pt = new PlaceholderTransition();
				pt.setCode(trimHeader.getCode());
				pt.setCodeSystem(trimHeader.getCodeSystem());
				pt.setCodeSystemName(trimHeader.getCodeSystemName());
				pt.setPath(trimHeader.getPath());
				pt.setLabel(transition.getLabel().getValue());
				pt.setFromStatus(transition.getFrom());
				pt.setToStatus(transition.getTo());
				pt.setTrimHeader(trimHeader);
				pt.setTransitionName(transition.getName());
				em.persist(pt);
			}
	...
	protected void obsoletePreviousTrimHeaders( TrimHeader trimHeader) {
		...
		Query query2 = em.createQuery("DELETE FROM PlaceholderTransition WHERE trimHeader.id IN (SELECT id FROM TrimHeader th WHERE name = :name " +
			"AND (status = 'active' OR status = 'new'))");
		query2.setParameter("name", trimHeader.getName());
		int results2 = query2.executeUpdate();
...
//		logger.info( "Adding phrase " + act.getTitle().getST().getValue());
	...
	//CCHIT merge
	/**
	 * New Method added to check for special characters
	 * @param aText
	 * @return
	 */
