...
import org.tolven.trim.BindMap;
...
import org.tolven.trim.ex.BindMapDefaultEx;
...
//		}
	...
	public void populateBindMaps(BindRequest request, MenuData md) {
		
		TrimExpressionEvaluator evaluator = new TrimExpressionEvaluator();
		evaluator.addVariable("trim", getTrim());
		String variable = computeVariable( request.rimObject );
		evaluator.addVariable(variable, request.rimObject);
		
		for (BindMap mp : request.placeholderBind.getMaps()) {
			
			String bindPath = mp.getPath();
			if (bindPath == null) {
				throw new RuntimeException("Missing <path> declaration in bind map of trim: " + getTrim().getName());
			}
			boolean isSlot = false;
			Object destinationValue = evaluator.evaluate("#{" + bindPath + "}");
			evaluator.addVariable("slot", destinationValue);
			
			if (destinationValue instanceof Slot) {
				isSlot = true;
				destinationValue = evaluator.evaluate("#{slot.value}");				
			}
			
			
			String bindName = mp.getName();
			if(bindName  != null) {
				TolvenLogger.debug( "in CreateScanner.bindToPlaceholder: map(name, path):" + bindName + ", " + bindPath, CreateScanner.class);
				//default the placeholder when the status is NEW
				if(md.getStatus() != null && md.getStatus().equals(Status.NEW)) {
					BindMapDefaultEx bmd = (BindMapDefaultEx) mp.getDefault();
					md.setField(bindName, bmd.getValue());
					if(bmd != null) {
						if (bmd.getValue() instanceof TS) {
							TSEx ats = (TSEx) bmd.getValue();
							if (ats.getFunction() != null && ats.getFunction().equalsIgnoreCase("now")) {
								ats.setDate(TolvenRequest.getInstance().getNow());
								md.setField(bindName, ats);
							}
						}
					}
				}
				
				if(destinationValue == null) { //If the value in the trim is already set, dont walk over it
					//Harvest
					Object phvalue = md.getField(bindName);
					if (isSlot) {
						evaluator.setValue("#{slot.value}", phvalue);
					} else {
						evaluator.setValue("#{" + bindPath + "}", phvalue);
					}
				}
			} else {
				String exp;
				...
				if(originalTrim != null) {
					...
					evaluator.pushContext(); //Switch context to the source trim
					
					evaluator.addVariable("trim", originalTrim);
					String uCallIt = bindPath.substring(0, bindPath.indexOf("."));
					if (!uCallIt.equalsIgnoreCase("trim")) {
						evaluator.addVariable(uCallIt, evaluator.evaluate("#{" + md.getDocumentPath() + "}"));
					}
					
					Object sourceValue = evaluator.evaluate(exp);
					
					evaluator.popContext(); //Now back to the destination trim
					
					evaluator.setValue(exp, sourceValue);
				} 
				
				//Re-check and see if the destination value became set, if not, default it
				destinationValue = evaluator.evaluate(exp);
				
				if (destinationValue == null) { //insert default value if the destinationValue was null
					BindMapDefaultEx bmd = (BindMapDefaultEx) mp.getDefault();
					
					//If the default was a function:
					if(bmd != null) {
						if (bmd.getValue() instanceof TS) {
							TSEx ats = (TSEx) bmd.getValue();
							if (ats.getFunction() != null && ats.getFunction().equalsIgnoreCase("now")) {
								ats.setDate(TolvenRequest.getInstance().getNow());
								evaluator.setValue(exp, ats);
							}
						} else {
							evaluator.setValue(exp, bmd.getValue());
						}
					}
				} 
	...
	public TrimEx getOriginalTrim() {
		return originalTrim;
	}

	public void setOriginalTrim(TrimEx originalTrim) {
		this.originalTrim = originalTrim;
	}
