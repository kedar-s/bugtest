...
public class ProcessTrim extends AppEvalAdaptor implements ProcessTrimLocal {
    ...
    private @EJB TolvenMessageSchedulerLocal tmSchedulerBean;
	...
	protected DocBase scanInboundDocument(DocBase doc) throws Exception {
		...
		TrimEx originalTrim = (TrimEx) getTrimMarshaller().unmarshal(tm.getXmlNS(), new ByteArrayInputStream(tm.getPayload()));
	    logger.info( "Trim Name in scanInboundDocument is  " + originalTrim.getMessage().getSender().getTrim());
		/**
		 * trim = getTrimBean().evaluateAndParseTrim(originalTrim.getMessage().
		 * getSender().getTrim(), variables); ExtensionScanner scanner = new
		 * ExtensionScanner(); scanner.setTrim( originalTrim );
		 * scanner.setTargetTrim( trim ); logger.info("Set target trim: " +
		 * trim.getName());
		 * scanner.scan();
		 */
		trim = originalTrim;
		convertToInshare(trim);	    
	...
	/**
	 * Convert the inbound "outShare" trim to be an inbound share trim
	 */
	public void convertToInshare(Trim trim) {
		TrimHeader customShareTrimH = getTrimBean().findTrimHeader(
				trim.getMessage().getSender().getTrim());
		StringReader is = new StringReader(new String(
				customShareTrimH.getTrim()));
		TrimEx customShareTrim = null;
		try {
			customShareTrim = (TrimEx) getTrimMarshaller().unmarshal(TRIMns, is);
			trim.setName(customShareTrim.getName());
			trim.setDescription(customShareTrim.getDescription());
			trim.setPage(customShareTrim.getPage());
			trim.setDrilldown(customShareTrim.getDrilldown());
		} catch (JAXBException e) {
			trim.setName("docclin/evn/sharePatientIn");
			trim.setDescription("Patient data from another account");
			trim.setPage("inShare.xhtml");
			trim.setDrilldown("inshareDD.xhtml");
		}
		trim.getTolvenEventIds().clear();
		for (Application app : trim.getApplications()) {
			if (app.getInstance().contains("patient:outShare")) {
			   app.setInstance(app.getInstance().replace("patient:outShare",
					"newInShare"));
			} else {
				// ccd trim may not have this
				app.setInstance(app.getName() + ":newInShare");
			}
		}
		for (BindTo bindTo : trim.getAct().getBinds()) {
			if (bindTo.getPlaceholder() != null) {
				bindTo.getPlaceholder().setPath(
						bindTo.getPlaceholder().getPath()
								.replace("outShare", "inShare"));
			}
		}
	...
	protected void loadWorkingMemory( StatefulSession workingMemory ) throws Exception {		
		if (trim == null) {
			trim = (TrimEx) getTrimMarshaller().unmarshal(tm.getXmlNS(), new ByteArrayInputStream(tm.getPayload()));
		}
	    ...
	    if (tm.getFromAccountId() == 0 || tm.getFromAccountId() == getAccount().getId()) {
	    	createPlaceholders((TrimEx)trim, BindPhase.CREATE);
	    }
...
*/        		if ("act/evn/treatmentPlan".equals(trim.getName()) && 
	...
	public void createPlaceholders(TrimEx trim, BindPhase bindPhase) {
		ExpressionEvaluator ee = getExpressionEvaluator();
		creatorBean.placeholderBindScan(getAccount(), trim, null, null, getNow(), bindPhase, this.getDocument(), null);
	}
	...
	public void send( Message message ) throws Exception {
		...
		TolvenMessage tmNew = null;
		if (getTm() instanceof TolvenMessageWithAttachments) {
        ...
        List<TolvenMessageAttachment> attachments = ((TolvenMessageWithAttachments) getTm()).getAttachments();
        for (TolvenMessageAttachment attachment : attachments) {
         TolvenMessageAttachment tma = new TolvenMessageAttachment();
         tma.setMediaType(attachment.getMediaType());
         tma.setDocumentId(attachment.getDocumentId());
         tma.setXmlNS(attachment.getXmlNS());
         tma.setPayload(attachment.getPayload());
         tma.setParent((TolvenMessageWithAttachments)tmNew);
         ((TolvenMessageWithAttachments)tmNew).getAttachments().add(tma);
        }
        tmSchedulerBean.queueTolvenMessage(tmNew);
