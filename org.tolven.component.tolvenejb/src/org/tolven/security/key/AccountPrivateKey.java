...
public class AccountPrivateKey extends TolvenEncryptedPrivateKey implements Serializable {
    ...
    public void init(AccountPrivateKey anAccountPrivateKey, PrivateKey aPrivateKey, PublicKey anEncryptionKey, String kbeKeyAlgorithm, int kbeKeyLength) throws GeneralSecurityException, IOException {
        ...
        if (anEncryptionKey == null) {
            throw new RuntimeException("A PublicKey is required to encrypt an AccountPrivateKey");
        }
    ...
    public PrivateKey getPrivateKey(PrivateKey aDecryptionKey) throws GeneralSecurityException, IOException {
        ...
        if (getEncodedEncryptedPrivateKeyInfo() != null) {
            ...
            accountPrivateKey = getKeyFactory(encryptedPrivateKeyInfo.getAlgName()).generatePrivate(keySpec);

        }
        if (getEncodedUnencryptedPrivateKey() != null) {
         // backward compatibility for V2.0, where keys were optionally encrypted, which is no longer the case in V2.1
            byte[] decryptedPrivateKey = getEncodedUnencryptedPrivateKey();
            PKCS8EncodedKeySpec keySpec = new PKCS8EncodedKeySpec(decryptedPrivateKey);
            accountPrivateKey = getKeyFactory(getAlgorithm()).generatePrivate(keySpec);
        }
