...
public class TolvenPerson implements Serializable {
    ...
    /**
     * Construct a TolvenPerson bean from an LDAP SearchResult. Note: We don't copy password from LDAP to TolvenPerson.
     */
    public TolvenPerson(SearchResult rslt) {
        // This is the primary key (called distinguishing name)
        setDn(rslt.getNameInNamespace());
        ...
        while (namingEnum.hasMoreElements()) {
            ...
            if ("userCertificate".equals(attrID)) {
                ...
                attr.add(obj);
                getAttributes().put("userCertificate;binary", attr);
            } else if (!"userPassword".equals(attrID)) {
    ...
    /**
     * Create a reasonable TolvenPerson from a virtualPerson.
     */
    public TolvenPerson(VirtualPerson vp) {
        setGivenName(vp.getFirst());
        setSn(vp.getLast());
        setCn(vp.getFirst() + " " + vp.getMiddle() + " " + vp.getLast());
        setGivenName(vp.getFirst());
        String uid = vp.getFirst() + "." + vp.getLast() + "@demo.org";
        addMail(uid);
        setUid(uid.toLowerCase());
        setUserPassword(vp.getFirst().toLowerCase());
    }

    /**
     * Set the distinquishing name (Primary key) for this entry.
     * In this case, we need the base DN and will use user id (UID) assuming it is unique.
     */
    public void generateDN(String baseDN) {
        this.setDn("uid=" + getUid() + "," + baseDN);
    }
    ...
    public Attributes dirAttributes(boolean changesOnly) {
        ...
        if (getUserPassword() != null && getUserPassword().length() > 0) {
            ...
            try {
                userPassword.add(SSHA.encodePassword(getUserPassword().getBytes()));
            } catch (Exception ex) {
    ...
    public String getPrimaryMail() throws NamingException {
    ...
    public byte[] getUserCertificate() {
        ...
        return (byte[]) getAttributeValue("userCertificate;binary");
    }

    public void setUserCertificate(byte[] userCertificate) {
        setAttributeValue("userCertificate;binary", userCertificate);
    }
