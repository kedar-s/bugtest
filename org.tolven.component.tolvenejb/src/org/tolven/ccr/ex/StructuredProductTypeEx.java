 ...
 *
 * @author Kul Bhushan
 * @version $Id: StructuredProductTypeEx.java,v 1.2 2009/05/19 18:29:20 jchurin Exp $
 */
...
public class StructuredProductTypeEx extends StructuredProductType{
    ...
    /**
     * Helper method to get productName text
     */
    public String getProductNameText( ) {
   		
    	List<StructuredProductType.Product> product = getProduct(); 
    	if (product==null || product.size()==0) return null;
    	org.tolven.ccr.CodedDescriptionType productName = product.get(0).getProductName();
    	if (productName==null) return null;
   		return productName.getText();
    }
    
    /**
     * Helper method to set productName text
     */
    public void setProductBrandNameText( String productName, String brandName ) {
   		    	
    	StructuredProductType.Product prodElement = CCRFactory.getInstance().createStructuredProductTypeProduct();
    	org.tolven.ccr.CodedDescriptionType product = CCRFactory.getInstance().createCodedDescriptionType();
    	org.tolven.ccr.CodedDescriptionType brand = CCRFactory.getInstance().createCodedDescriptionType();
    	product.setText(productName);
    	prodElement.setProductName(product);
    	brand.setText(brandName);
    	prodElement.setBrandName(brand);
    	getProduct().add(prodElement);
    }
    
    /**
     * Helper method to get productName text
     */
    public String getProductBrandNameText( ) {
   		
    	List<StructuredProductType.Product> product = getProduct(); 
    	if (product==null || product.size()==0) return null;
    	org.tolven.ccr.CodedDescriptionType brandName = product.get(0).getBrandName();
