...
public class RuleQueueBean implements RuleQueueLocal {
        ...
        try {
            ...
            for (int i = 0; i < payloads.size(); i++) {
                ...
                if (isTolvenMessage) {
                    ...
                    if (0 != tm.getId()) {
                    	// This is a TolvenMessage that has data stored in the payload
                    	TolvenMessageInfo info = new TolvenMessageInfo(tm.getId());
                    	message = session.createObjectMessage(info);
                    }
                    else {
                    	// This is a TolvenMessage that does not have data stored in the payload
    ...
    public QueueContext getQueueContext(){
         ...
         try {
             ...
             QueueContext qc =  (QueueContext)tolvenContext.getQueueContext(getQueueName());
             return qc;
         } catch (Exception ex) {
