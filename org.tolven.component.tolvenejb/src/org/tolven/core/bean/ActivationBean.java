...
public class ActivationBean implements ActivationLocal {
       ...
       */
       public List<AccountUser> findAccountUsers( Account account ) {
           ...
           Query query = em.createQuery("SELECT au FROM AccountUser au WHERE au.account = :account " +
                  "and (au.user.status = '" + activeStatus + "' or au.user.status = '" + oldActiveStatus + "') order by au.user.ldapUID, au.effectiveDate");       
