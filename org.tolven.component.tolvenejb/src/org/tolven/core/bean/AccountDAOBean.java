...
public class AccountDAOBean implements AccountDAOLocal {
    ...
    /**
     * Create a template Account. Template accounts have no users and no accountTemplate of their own.
     * @param accountType
     * @return
     */
    @Override
    ...
    */
    ...
    public AccountUser inviteAccountUser(Account account, AccountUser inviterAccountUser, TolvenUser invitedUser, String invitedUserRealm, PrivateKey anInviterUserPrivateKey, Date now, boolean accountPermission, boolean isReinvite) {
        ...
        try {
            ...
            return addAccountUser(invitedAccountUser, account, invitedUser, invitedAccountPrivateKey, now, accountPermission, invitedUserPublicKey);
        } catch (Exception ex) {
   ...
   /* public AccountUser getSystemAccountUser(Account account, boolean createIf, Date now) {
        String principal = (String) TolvenSessionWrapperFactory.getInstance().getPrincipal();
        AccountUser accountUser = findAccountUser(principal, account.getId());
        if (accountUser == null) {
            ...
            TolvenUser tolvenUser = getActivationBean().findUser(principal);
            if (tolvenUser == null) {
                //Activate the user
                TolvenPerson tp = new TolvenPerson();
                tp.setUid(principal);
                tolvenUser = getLoginBean().activate(tp, TolvenRequest.getInstance().getNow());
            }
            accountUser = findSystemAccountUser(tolvenUser, account, status);
            if (accountUser == null) {
                if (createIf) {
     ...
     * @param accountPermission boolean indicating if this user has account administration permission
     */
    private AccountUser addAccountUser(AccountUser invitedAccountUser, Account account, TolvenUser user, AccountPrivateKey accountPrivateKey, Date now, boolean accountPermission, PublicKey invitedUserPublicKey) {
        ...
        setupAccountKeys(account, au, accountPrivateKey, user, invitedUserPublicKey);
        em.persist(au);
        String principal = (String) TolvenSessionWrapperFactory.getInstance().getPrincipal();
     ...
     */
    public void setupAccountKeys(Account account, AccountUser accountUser, AccountPrivateKey accountPrivateKey, TolvenUser user, PublicKey invitedUserPublicKey) {
        try {
            if (invitedUserPublicKey == null) {
                String message = null;
                if (account.getId() == 0) {
            ...
            if (account.hasPublicKey()) {
                if (accountPrivateKey == null) {
                    throw new RuntimeException("An accountPrivateKey is required: " + account.getId());
                } else {
                    accountUser.setAccountPrivateKey(accountPrivateKey);
                }
            } else {
                accountPrivateKey = AccountPrivateKey.getInstance();
    ...
    */
    ...
    public UserKeyLocal getUserKeyBean() {
        return userKeyBean;
    }

    public void setUserKeyBean(UserKeyLocal userKeyBean) {
	...
	@Override
	public boolean addSystemUserToAccount(AccountUser activeAccountUser,PrivateKey userPrivateKey) {
		//Find out the username for mdbuser
        QueueContext qc = ruleQueueLocal.getQueueContext();
        //Get public key for mdbuser
        //find tolvenuser for mdbuser
        //add mdbuser to the account
		inviteAccountUser(activeAccountUser.getAccount(), activeAccountUser, qc.getUser(),qc.getRealm(), userPrivateKey, new Date(), false, false);
		return false;
	}
} //endclass
