...
public class TestExtensionScanner extends XMLTestBase {
	...
	public void testTrimMerge() throws JAXBException, IOException {
		JAXBContext jc = setupJAXBContext("org.tolven.trim");
		InputStream patient1Stream = getClass().getResourceAsStream("patient1.trim.xml");
		Trim patient1 = (Trim) unmarshalStream(jc, patient1Stream, factory);
		InputStream patient2Stream = getClass().getResourceAsStream("patient2.trim.xml");
		Trim patient2 = (Trim) unmarshalStream(jc, patient2Stream, factory);
		ExtensionScanner scanner = new ExtensionScanner();
		scanner.setTrim( patient2 );
		scanner.setTargetTrim( patient1 );
		scanner.scan();
		logger.info( this.marshal(jc, patient1) );
	}
}
