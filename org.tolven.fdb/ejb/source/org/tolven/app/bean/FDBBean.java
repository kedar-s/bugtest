 ...
 *  on the FDB, u just need to write the declarations of that method in FDBInterface and then define
 *  it in this class.
 */
...
public class FDBBean implements FDBInterface {
	...
	@Override
	public  Map<String,String> searchDrugExhaustive(long drugId ,String priorMedicines , String priorAllergies , String dob)throws Exception{
		...
		try{
			...
			Screening screening = new Screening(fdbManager);
			...
			StringBuffer drugDrugInteractions = new StringBuffer();
			if(!StringUtils.isBlank(priorMedicines)){
				scDrugs = generateScreenDrugList(priorMedicines);
				for(int i=0;i<scDrugs.count();i++){
					ScreenDrugs drugsList = new ScreenDrugs(fdbManager);
					drugsList.addItem(scDrugs.get(i));
					drugsList.addItem(scDrug);
					drugDrugInteractions.append(doScreenDDIM(drugsList, screening));
				}				
			}			
			// Drug / Food Interaction
			ArrayList<String> drugFood = doScreenDFIM(scDrug ,screening);
			
			// Duplicate Therapy Checking
			ArrayList<String> duplicateTherapy = doScreenDT(scDrug ,screening);
			
			//Drug Allergy Interaction
			String drugAllergy = new String();
			if(null != priorAllergies && priorAllergies.trim().length() > 0){
				drugAllergy = doScreenDAI(scDrug ,screening,  priorAllergies);
			}
    	    ...
    	    if(currentDrug.getStatusCode().equals("0")){
    	    	//consolidate the classifications for the drug
		    	if(null != currentDrug.getClassifications(FDBClassificationsType.fdbCTActionGroup) &&
		    		...
		    		for(int i=0; i< classifications.count(); i++){
		    			CTActionGroupClassification +=classifications.item(i).getDescription();
		    			if(i>0)
		    				CTActionGroupClassification +=",";		    				
		    		}		    		
		    		fdbResponseBuffer.append("Action Group CLASSIFICATIONS FOR THE DRUG :"+CTActionGroupClassification+"|");		    		
		    	}
		    	if(null != currentDrug.getClassifications(FDBClassificationsType.fdbCTETC) &&
		    		...
		    		fdbResponseBuffer.append("CTETC CLASSIFICATIONS FOR THE DRUG : "+CTETCClassification).append("|");
		    	}
		    	if(null != currentDrug.getClassifications(FDBClassificationsType.fdbCTFDB) &&
		    		...
		    		fdbResponseBuffer.append("CTFDB CLASSIFICATIONS FOR THE DRUG : "+CTFDBClassification+"|");
		    	}
		    	if(null != currentDrug.getClassifications(FDBClassificationsType.fdbCTAHFS) &&
		    		...
		    		fdbResponseBuffer.append("AHFS CLASSIFICATIONS FOR THE DRUG :"+AHFSClassification+"|");
		    	}
			...
			if(drugDrugInteractions.length() > 0){
				...
				fdbResponseBuffer.append(drugDrugInteractions.toString());
				fdbResponseBuffer.append("|");				
			}
			if(drugFood.size() > 0){
			...
			if(drugAllergy.length() > 0){
				hasDrugAllergy = "true";
				fdbResponseBuffer.append(drugAllergy);
				fdbResponseBuffer.append("|");				
			}       
			...
			responseMap.put("fdbResponse", fdbResponse);//responseArray.add(0, fdbResponse);
			responseMap.put("hasDrugDrugInterraction",hasDrugDrugInterraction);//responseArray.add(1,hasDrugDrugInterraction);
			responseMap.put("hasDrugFoodInterraction",hasDrugFoodInterraction);//responseArray.add(2,hasDrugFoodInterraction);
			responseMap.put("hasDuplicateTherapy",hasDuplicateTherapy);//responseArray.add(3,hasDuplicateTherapy);
			responseMap.put("drugDrugResponse",drugDrugInteractions.toString());//	responseArray.add(4, drugDrugResponse.toString());
			responseMap.put("drugFoodResponse",drugFoodResponse.toString());//responseArray.add(5, drugFoodResponse.toString());
			responseMap.put("duplicateTherapyResponse",duplicateTherapyResponse.toString());//responseArray.add(6, duplicateTherapyResponse.toString());
			responseMap.put("hasDrugAllergy",hasDrugAllergy.toString());//responseArray.add(7, hasDrugAllergy);
			responseMap.put("drugAllergyResponse",drugAllergy);//responseArray.add(8, drugAllergyResponse.toString());
			responseMap.put("strength",strength);//responseArray.add(9, strength);
			responseMap.put("dose",dose);//responseArray.add(10, dose);
			responseMap.put("route",route);//responseArray.add(11, route);
			responseMap.put("maxDose",String.valueOf(maxDose));//responseArray.add(12, String.valueOf(maxDose));
			responseMap.put("AHFSClassification",AHFSClassification);//responseArray.add(13, AHFSClassification);
			responseMap.put("CTETCClassification",CTETCClassification);//responseArray.add(18, CTETCClassification);
			responseMap.put("CTFDBClassification",CTFDBClassification);//responseArray.add(19, CTFDBClassification);
			responseMap.put("CTActionGroupClassification",CTActionGroupClassification);//responseArray.add(20, CTActionGroupClassification);			
			responseMap.put("prescriberInstructions",prescriberInstructions.toString());//responseArray.add(14, prescriberInstructions.toString());
			responseMap.put("patientInstructions",patientInstructions.toString());//responseArray.add(15, patientInstructions.toString());
			responseMap.put("commonOrders",commonOrders.toString()); //responseArray.add(16, commonOrders.toString());
			responseMap.put("poem",poem.toString());//responseArray.add(17, poem.toString());
			responseMap.put("isControlled",isControlled);//responseArray.add(21, isControlled);
			responseMap.put("doseForm",doseForm);//responseArray.add(22, doseForm);
			if (currentDrug != null && currentDrug.getStrengthUnit()!=null)
				...
				responseMap.put("strengthUnit",currentDrug.getStrengthUnit());//responseArray.add(23, currentDrug.getStrengthUnit());
			return responseMap;
		...
		if(null != priorMedicines && priorMedicines.trim().length() > 0){
			...
			try {
				for(int i=0; i<drugs.length; i++){
					if(StringUtils.isBlank(drugs[i]))
						continue;
	 ...
	 */
	private String doScreenDDIM(ScreenDrugs drugs, Screening objScreen) throws FDBSQLException {
        long lngCnt;
        FDBDDIMSeverityFilter sev = FDBDDIMSeverityFilter.fdbDDIMSFModerate ;
        DDIMScreenResults objResultsDDIM = objScreen.DDIMScreen(drugs,false,sev, false, false);   
		
        // Retrieve any of the messages generated by this screening
     	//Messages objMessages =objResultsDDIM.getMessages ();
        ...
        if (!(lngCnt >= 0)){
        	message.append("No Drug - Drug Interractions Found.");
        } else {
           ...
           // messagesList.add("The following Drug - Drug Interractions were Found.");
            for (int intInd = 0; intInd <= (int)lngCnt - 1; intInd++){
              ...
              message.append("DDIM : "+objDDIMRes.getDrug1Description() + " and " + objDDIMRes.getDrug2Description());
              message.append("		DDIM Result #" + Integer.toString(intInd + 1));
              message.append("		" + objDDIMRes.getDrug1Description() + " and " + objDDIMRes.getDrug2Description());
              message.append("		Interaction Description: " + objDDIMRes.getInteractionDescription());
              message.append("		Message: " + objDDIMRes.getScreenMessage());             
            }
        }
...
*/
	 ...
	 * @throws Exception 
	 */
	public ArrayList<AllergyVO> fetchDrugAllergies(int offset,String filter){
		ArrayList<AllergyVO> arrayList= null;
		StringBuffer buff = new StringBuffer("SELECT allgpcick FROM FdbAllergenpicklist allgpcick ");
		
		if(!StringUtils.isBlank(filter)){
			buff.append(" where allgpcick.description1 like :filter ");
			
		}
		buff.append(" ORDER BY allgpcick.description1");	
		Query query = getEm().createQuery(buff.toString());
		if(!StringUtils.isBlank(filter)){
        ...
        for(FdbAllergenpicklist allergenpick:allergenpicklists){
        	...
        	allerVO.setInternalId(Long.parseLong(allergenpick.getId().getConceptid().toString()));
	...
	public List<AllergyVO> fetchFilteredAllergies(String filter , int offset) throws Exception{
		...
		sf.setPhoneticSearch(searchType);
		sf.setSearchMethod(searchMethod);
		//if no filter text is found set filter to *
		filter = StringUtils.isBlank(filter)?"*":filter;		
		AllergenPicklistItems allergens = navigate.allergenPicklistSearch(filter, sf, false);
		for(int i=offset; i< allergens.count(); i++){
			...
			allergVO.setInternalId(Long.parseLong(allergens.item(i).getID()));
			list.add(allergVO);
			if (list.size() == 14) {
				break;
			}
	 ...
	 * @throws Exception 
	 */
	private String doScreenDAI(ScreenDrug scDrug,Screening screening, String priorAllergies) throws Exception {
		Messages objMessages =null;
        ...
        try {
			...
			drugAllergyResponse.append("No Drug - Allergy Interractions Found.");
		}
        ...
        if (!(lngCnt >= 0)){
            ...
            for (int intInd = 0; intInd <= (int)lngCnt - 1; intInd++){
              ...
              drugAllergyResponse.append(" Warning : "+warning);
              drugAllergyResponse.append(" DAM : "+objDAMMRes.getDrugDescription());
              drugAllergyResponse.append(" DAM Result #" + Integer.toString(intInd + 1));
              drugAllergyResponse.append(" Allergen Name : " + objDAMMRes.getAllergenDescription());
              drugAllergyResponse.append(" Reaction : " + objDAMMRes.getScreenMessage());
              drugAllergyResponse.append(" Match Description : " + objDAMMRes.getMatchDescription());
            }
        }
        return drugAllergyResponse.toString();
	}
	...
	private ScreenAllergies createAllergens(String priorAllergies) throws Exception {
		String[] allergies = priorAllergies.split(",");		
		SearchFilter sf = new SearchFilter();
		FDBSearchMethod searchMethod = FDBSearchMethod.fdbSMSimple;
		...
		if(null !=allergies){
			for(int i=0; i<allergies.length; i++){
				if(StringUtils.isBlank(allergies[i]))
					continue;
	...
	public long toAgeString( String dob) {
		...
		try {
			dateOfBirth = HL7DateFormatUtility.parseDate(dob);
...
//		fs.getDatabaseIssueDate();
			...
			if(currentDrug.getStatusCode().equals("0")){
		    	...
		    	for(int i=0; i< pack.count(); i++){
		    		...
		    		/*if(pack.item(i).getRepackager() == true || 
		    			...
		    			ndcDetails.put("ndcCodeQual", propertyBean.getProperty("ncpdp.productCodeQualifier"));
		    			ndcDetails.put("codeSystem", propertyBean.getProperty("ncpdp.externalcode.FDB")); //NCPDP external code for FirstDataBank MedicationID
	...
	public FdbAllergenpicklist findDrugAllergy(FdbAllergenpicklistPK pk){
		return (FdbAllergenpicklist)getEm().find(FdbAllergenpicklist.class, pk);
	}
