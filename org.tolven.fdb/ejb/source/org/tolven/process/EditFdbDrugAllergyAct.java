...
public class EditFdbDrugAllergyAct extends InsertAct {
	...
	public void compute() throws Exception {
		...
		if (isEnabled() && StringUtils.isNotBlank(getTemplate())) {		
			...
			if (getAction().equals("addDrugAllergyDetails")){
				...
				FdbAllergenpicklist allergy = fdbInterface.findDrugAllergy(getAllergyId());
				CE ce = new CE();
				ce.setCodeSystem("FDB");
				ce.setCodeSystemVersion(fdbInterface.getFdbVersion().getDbversion());
				ce.setDisplayName(allergy.getDescription1());
				ce.setCode(constructAllergyCode(allergy.getId()));
				this.getTrim().getAct().getObservation().getValues().get(0).setCE(ce);
			}
	...
	private String constructAllergyCode(FdbAllergenpicklistPK pk){
		return pk.getConceptid()+"_"+pk.getConcepttype();
	}
	...
	private FdbAllergenpicklistPK getAllergyId(){
		...
		pk.setConceptid(new Integer(getFdbDrugAllergyCode()));
		pk.setConcepttype(new Integer(getFdbDrugAllergyType()));
		return pk;
	}
}
