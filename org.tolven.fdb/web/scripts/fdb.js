	...
	if(sourceType == "selectDrugAllergy"){ 
		var myAjax = new Ajax.Request('allergyUpdate15.ajaxfdb', 
				{method: 'get', 
				 ...
				 onFailure: function() {hideProcessing(formId);displayError(request,formId);},
				 onComplete: function(response) {
						try{
			 ...
			 onFailure: function() {hideProcessing(formId);displayError(request,formId);},
			  onComplete: function(response) {
					...
					$(formId+":rowId").value = resp.offSet;
						...
						alert("Error loading values from FDB in fdb.js:loadValuesFromFDB "+e);
					}
					hideProcessing(formId);
				}
			});	
...
//methodArgs - {formId,destGridID,element,template,favoriteItem}
function addFDBDataToFavorites(code,isDelete,methodArgs,conceptType){
	var params = methodArgs.split(',');
	var destGridId = params[1];
	var formId = params[0];
	if(isDelete == true || isDelete == 'true' ){
	  // remove the selected items from the right hand side grid
		// and trigger ajax to remove the act.
		var selectedItems = $$("tr#"+code);
		for(i=0;i<selectedItems.length;i++){
			$(selectedItems[i]).remove();
		}
		$(formId + ":computeDelete").value = true;
	}else{
		$(formId + ":computeDelete").value = false;
		 // copy the selected items from the left hand side grid
		// and trigger ajax to add the act.
		  var destGrid = $(destGridId+"_grid");
		  var selectedItems ;
		  if(!conceptType)
			  selectedItems = $$("a#"+code);
		  else
			  selectedItems = $$("a#"+code+"_"+conceptType);
		  ...
		  for(i=0;i<selectedItems.length;i++){
			...
			row.id= code;
			$(row).update("<td onclick=javascript:addFDBDataToFavorites('"+code+"',true,'"+destGridId+"','"+params[2]+"')>"+selectedItems[i].innerHTML+"</td>");
		  }
	}
	//rewrite the styles
	...
	//trigger compute
	$(formId + ":computeEnable").value = "true";
	$(formId + ":computeFdbCode").value = code;
	$(formId + ":computeListItemId").value = params[4];
	if(conceptType)
		$(formId + ":computeConceptTypeId").value = conceptType;
	var rootForm = $(formId);
	var wipNode = rootForm.parentNode;
	ajaxSubmit4(rootForm, wipNode.id, 1);
	
	//<h:inputHidden id="computeConceptTypeId"  value="#{trim.trim.act.computes[0].property['conceptTypeId']}"/>			
    //<h:inputHidden id="computeListItemId"  value="#{trim.trim.act.computes[0].property['listItemId']}"/>	
}