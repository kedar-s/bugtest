...
public class LoadDiagnosis extends LoadRESTfulClient {
	String fileName;
    ...
    public LoadDiagnosis(String userId, char[] password, String appRestfulURL, String authRestfulURL) {
        init(userId, password, appRestfulURL, authRestfulURL);
    }
    
	public String generateName( String fields[] ) {
		return "diagnosis/SCTID-" + fields[SCTID];
	}
...
//
		...
		writer.writeCharacters("obs/evn/diagnosis");
 ...
 */
	...
	public void load(String fileName) throws Exception {
		TolvenLogger.info("Uploading UMLS Diagnosis list from: " + fileName, LoadDiagnosis.class);
        ...
        while (reader.ready()) {
            ...
            if (rowCount > getIterationLimit()) {
        		TolvenLogger.info( "Upload stopped early due to " + UPLOAD_LIMIT + " property being set", LoadDiagnosis.class);
            	break;
            }
...
//			TolvenLogger.info( "Load Trim: " + trimName, LoadDiagnosis.class );
            createTrimHeader(bos.toString());
			TolvenLogger.info(bos.toString(), LoadDiagnosis.class);
        }
        // Send unfinished batch, if any.
		TolvenLogger.info( "Count of Diagnoses uploaded: " + (rowCount-1), LoadDiagnosis.class);
		TolvenLogger.info( "Activating headers... ", LoadDiagnosis.class);
        activate();
...
<trim xmlns="urn:tolven-org:trim:4.0">
	...
	<name>diagnosis/SCTID-274945004</name>
...
*/
	...
	public static void main(String[] args) throws Exception {
		...
		//LoadDiagnosis ld = new LoadDiagnosis(args[0]);
