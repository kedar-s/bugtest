...
public class CCHITMenuAction extends MenuAction {
	...
	public Map<String, String> getMenuItemMap(String path, String role) {
		...
		// get menu structure
    	AccountMenuStructure ams = getMenuLocal().findAccountMenuStructure(activeAccountUser.getAccount().getId(), path );
    	MenuStructure ms = getMenuLocal().findMenuStructure(activeAccountUser, ams);
    	// get its preferred children
    	List<MenuStructure> children = getMenuLocal().findSortedChildren(activeAccountUser, ams);
	...
	public MenuDataList getAllMenuData() throws Exception {
		if (menuDataList==null) {
			...
			menuDataList = new MenuDataList( this.getMenuLocal(), this.getAccountId(), 
					this.getElement(), getNow(),activeAccountUser);
		}
		return menuDataList;
	}
}
