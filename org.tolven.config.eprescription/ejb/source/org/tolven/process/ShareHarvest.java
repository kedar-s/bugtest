 ...
 * @author John Churin
 */
public class ShareHarvest extends ComputeBase {
	...
	public String buildFullPath( String path ) {
		...
		if (path==null || path.length() == 0 || ':'==path.charAt(0)) {
			if (getAccountUser().getAccount().getAccountType().getKnownType()
					.equals("ephr")) {
				return getAccountUser().getAccount().getAccountType()
						.getKnownType()
	...
	public List<MenuData> getUnsharedMenuData() throws Exception{
    	...
    	MenuPath pathContext = new MenuPath( this.getPathContext() );
    	String menuStructurePath = buildFullPath(this.getPath());
		MenuPath requestPath = new MenuPath( menuStructurePath, pathContext);
