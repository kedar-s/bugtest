 ...
 */
showPharmacyDropDown=function(elementId,path){
    new Ajax.Updater(elementId, '../wizard/dropDownPharmacyTemplate.jsf', {
           parameters: { element: path }, onComplete:function(req) {wizHideAjaxLoader();}
    });
}

function showPrescriberWarningDialog(formId,path){
	new Ajax.Updater(formId+':warningDialog', '/Tolven/wizard/prescriberWarning.jsf', {
		parameters: { element: path }, 
		evalScripts:true, 
		onComplete:function(req) {
			$(formId+':warningDialog').show();
    	}
	});	
}	
function closePrescriberWarningDialog(formId){
	$(formId+':warningDialog').hide();
}
...
/**
 * select drug from the popup in the medication order wizard 
 * @param obj
 * @param formId
 * @param element
 * @param isFormulary
 * @return
 */
function selectDrugInMedicationDocument(code , methodArgs , element) {
	var params = methodArgs.split('|');
	var formId = params[0];
	$(formId+":ajaxLoader").style.display="block";
	
	var rootForm = $(formId);
	$(formId + ":drugCode").value = code;
	$(formId + ":computeEnable").value = "true";
	var wipNode = rootForm.parentNode;
	var currentStep = 1 * rootForm.getAttribute('currentStep');
	ajaxSubmit4(rootForm, wipNode.id,currentStep);
...
function addFdbDrugToFormulary(code,methodArgs){
	var params = methodArgs.split('|');
	var formId = params[0];
	var rootForm = $(formId);
...
function startEndDate(root , element){
	if(null != $(root+":daysSupply").value){
		...
		if(startDay.length ==0){
			...
			startDay = (currentDate.getUTCMonth()+1)+"/"+currentDate.getUTCDate()+"/"+currentDate.getUTCFullYear();
			$(root+":FieldqueseffectiveTimeLow").value = startDay;
		}			
		var days = $(root+":daysSupply").value;	
		var startDate = new Date(startDay);
		startDate.setDate(startDate.getUTCDate()+parseInt(days));
		$(root+":FieldqueseffectiveTimeHigh").value = (startDate.getUTCMonth()+1)+"/"+startDate.getUTCDate()+"/"+startDate.getUTCFullYear();
	}
}

function overwriteInteractionText(formId){
	var rootForm = $(formId);
	var wipNode = rootForm.parentNode;
	var currentStep = 1 * rootForm.getAttribute('currentStep');
	$(formId + ":enableOverride").value = "true";
	ajaxSubmit4(rootForm, wipNode.id,currentStep-1);
}
