...
public class RecoverLoginPasswordAction {
    ...
    public String findSecurityQuestion() {
        try {
            activeSecurityQuestion = getLoginPasswordLocal().findActiveSecurityQuestion(getSearchUid(), getSearchRealm());
            setUid(getSearchUid());
    ...
    public String recoverLoginPassword() {
        ...
        try {
            ...
            getLoginPasswordLocal().recoverPassword(getUid(), getNewUserPassword().toCharArray(), getRealm(), getActiveSecurityQuestion(), sqa);
        } catch (AuthenticationException ex) {
            ex.printStackTrace();
