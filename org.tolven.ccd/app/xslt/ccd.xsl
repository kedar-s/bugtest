...
<!-- <xsl:variable name="totalInactiveAllergyNodes" select="count($allergies_inactive/results/rows/child::node())" /> -->
<xsl:variable name="totalActiveProblemNodes" select="count($problems_active/results/rows/child::node())" />
<xsl:variable name="totalInactiveProblemNodes" select="count($problems_inactive/results/rows/child::node())" />
<xsl:variable name="totalResolvedProblemNodes" select="count($problems_resolved/results/rows/child::node())" />
	...
	<recordTarget>
		<patientRole>
			<!-- <id root="ProviderID" extension="PatientID" assigningAuthorityName="Provider Name"/> -->
			 ...
			 <xsl:if test="starts-with(@extension, 'echr:patient')">
			 	<id>
		...
		<structuredBody>
			<component>
				...
				<section>
							...
							<tbody>
								<xsl:if test="$totalActiveAllergyNodes > 0">
									<xsl:call-template name="AllergySummary">
										...
										<xsl:with-param name="totalAllergyNodes" select="$totalActiveAllergyNodes"/>
										<xsl:with-param name="allergyNodeCnt" select="1"/>
										<xsl:with-param name="allergyNodeCntDisplay" select="1"/>
										<xsl:with-param name="summaryMode" select="'active'"/>
									</xsl:call-template>	
								</xsl:if>	
								<!-- <xsl:if test="$totalInactiveAllergyNodes > 0">
									<xsl:call-template name="AllergySummary">
										<xsl:with-param name="allergyNode" select="$allergies_inactive/results/rows"/>
										<xsl:with-param name="totalAllergyNodes" select="$totalInactiveAllergyNodes"/>
										<xsl:with-param name="allergyNodeCnt" select="1"/>
										<xsl:with-param name="allergyNodeCntDisplay" select="$totalActiveAllergyNodes + 1"/>
										<xsl:with-param name="summaryMode" select="'inactive'"/>
									</xsl:call-template>	
								</xsl:if>	 -->		
								 <xsl:if test="$totalActiveAllergyNodes = 0">
									<tr >
					...
					<xsl:choose>
						<xsl:when test="$totalActiveAllergyNodes > 0">
							<xsl:call-template name="AllergyDetail">
								...
								<xsl:with-param name="allergyDetailNodeCntDisplay" select="1"/>
								<xsl:with-param name="summaryMode" select="'active'"/>
							</xsl:call-template>
						</xsl:when>
						<xsl:otherwise>
							<xsl:call-template name="AllergyDetailBlank"/>
						</xsl:otherwise>
					</xsl:choose>									
					<!-- <xsl:if test="$totalInactiveAllergyNodes > 0">
						<xsl:call-template name="AllergyDetail">
							<xsl:with-param name="allergyDetailNode" select="$allergies_inactive/results/rows"/>
							<xsl:with-param name="totalAllergyNodes" select="$totalInactiveAllergyNodes"/>
							<xsl:with-param name="allergyDetailNodeCnt" select="1"/>
							<xsl:with-param name="allergyDetailNodeCntDisplay" select="$totalActiveAllergyNodes + 1"/>
							<xsl:with-param name="summaryMode" select="'inactive'"/>
						</xsl:call-template>	
					</xsl:if>	 -->			
				</section>
			...
			<component>
				...
				<section>
							...
							<tbody>
								<xsl:if test="$totalActiveProblemNodes > 0">
									<xsl:call-template name="ProblemSummary">
										...
										<xsl:with-param name="totalProblemNodes" select="$totalActiveProblemNodes"/>
										<xsl:with-param name="problemNodeCnt" select="1"/>
										<xsl:with-param name="problemNodeCntDisplay" select="1"/>
										<xsl:with-param name="summaryMode" select="'active'"/>
									</xsl:call-template>	
								</xsl:if>
								<xsl:if test="$totalInactiveProblemNodes > 0">
									<xsl:call-template name="ProblemSummary">
										<xsl:with-param name="problemNode" select="$problems_inactive/results/rows"/>
										<xsl:with-param name="totalProblemNodes" select="$totalInactiveProblemNodes"/>
										<xsl:with-param name="problemNodeCnt" select="1"/>
										<xsl:with-param name="problemNodeCntDisplay" select="$totalActiveProblemNodes + 1"/>
										<xsl:with-param name="summaryMode" select="'inactive'"/>
									</xsl:call-template>	
								</xsl:if>	
								<xsl:if test="$totalResolvedProblemNodes > 0">
									<xsl:call-template name="ProblemSummary">
										<xsl:with-param name="problemNode" select="$problems_resolved/results/rows"/>
										<xsl:with-param name="totalProblemNodes" select="$totalResolvedProblemNodes"/>
										<xsl:with-param name="problemNodeCnt" select="1"/>
										<xsl:with-param name="problemNodeCntDisplay" select="$totalActiveProblemNodes + $totalInactiveProblemNodes + 1"/>
										<xsl:with-param name="summaryMode" select="'resolved'"/>
									</xsl:call-template>	
								</xsl:if>							
								<xsl:if test="$totalActiveProblemNodes = 0 and $totalInactiveProblemNodes = 0 and $totalResolvedProblemNodes = 0">
									<tr >
									<td />
									<td />
									<td />
					...
					<xsl:choose>
						<xsl:when test="$totalActiveProblemNodes > 0">
							<xsl:call-template name="ProblemDetail">
								...
								<xsl:with-param name="problemDetailNodeCntDisplay" select="1"/>
								<xsl:with-param name="summaryMode" select="'active'"/>
							</xsl:call-template>	
						</xsl:when>
						<xsl:when test="$totalInactiveProblemNodes > 0">
							<xsl:call-template name="ProblemDetail">
								<xsl:with-param name="problemDetailNode" select="$problems_inactive/results/rows"/>
								<xsl:with-param name="totalProblemNodes" select="$totalInactiveProblemNodes"/>
								<xsl:with-param name="problemDetailNodeCnt" select="1"/>
								<xsl:with-param name="problemDetailNodeCntDisplay" select="$totalActiveProblemNodes + 1"/>
								<xsl:with-param name="summaryMode" select="'inactive'"/>
							</xsl:call-template>	
						</xsl:when>	
						<xsl:when test="$totalResolvedProblemNodes > 0">
							<xsl:call-template name="ProblemDetail">
								<xsl:with-param name="problemDetailNode" select="$problems_resolved/results/rows"/>
								<xsl:with-param name="totalProblemNodes" select="$totalResolvedProblemNodes"/>
								<xsl:with-param name="problemDetailNodeCnt" select="1"/>
								<xsl:with-param name="problemDetailNodeCntDisplay" select="$totalActiveProblemNodes + $totalInactiveProblemNodes + 1"/>
								<xsl:with-param name="summaryMode" select="'resolved'"/>
							</xsl:call-template>	
						</xsl:when>	
						<xsl:otherwise>
							<xsl:call-template name="ProblemDetailBlank"/>
						</xsl:otherwise>
					</xsl:choose>
			...
			<component>
				...
				<section>
					...
					<xsl:choose>
						<xsl:when test="$totalMedicationNodes > 0">
						...
						<xsl:otherwise>
							<xsl:call-template name="MedicationDetailBlank"/>
						</xsl:otherwise>
					</xsl:choose>
				</section>
			</component>
			<component>
				...
				<section>
					...
					<xsl:choose>
						...
						<xsl:when test="$totalLabResultNodes > 0">
						...
						<xsl:otherwise>
							<xsl:call-template name="LabResultDetailBlank"/>
						</xsl:otherwise>
					</xsl:choose>
			...
			<component>
				...
				<section>
					...
					<xsl:choose>
						...
						<xsl:when test="$totalProcedureNodes > 0">
						...
						<xsl:otherwise>
							<xsl:call-template name="ProcedureDetailBlank"/>
						</xsl:otherwise>
					</xsl:choose>					
				</section>
			</component>
...
<xsl:template name="AllergySummary" xmlns="urn:hl7-org:v3">
	...
	<xsl:param name="allergyNodeCntDisplay" select="0"/>
	<xsl:param name="totalAllergyNodes" select="0" />
	<xsl:param name="summaryMode"/>
	<xsl:variable name="allergyPath" select="document($allergyNode/child::node()[position()=$allergyNodeCnt]/drilldown)"/>
	<tr>
		<xsl:attribute name="ID">
			<xsl:value-of select="concat('ALGSUMMARY_', $allergyNodeCntDisplay)" />
		</xsl:attribute>	
		<td>
			<xsl:attribute name="ID">
				<xsl:value-of select="concat('ALGTYPE_', $allergyNodeCntDisplay)" />
			</xsl:attribute>	
			416098002 -- Drug Allergy (disorder)
		</td>
		<td>
			<xsl:attribute name="ID">
				<xsl:value-of select="concat('ALGSUB_', $allergyNodeCntDisplay)" />
			</xsl:attribute>
		...
		<td>
			<xsl:attribute name="ID">
				<xsl:value-of select="concat('ALGREACT_', $allergyNodeCntDisplay)" />
			</xsl:attribute>						
			<xsl:value-of select="$allergyPath/results/rows/row/reactions"/>			
		</td>
		<td>
			<xsl:attribute name="ID">
				<xsl:value-of select="concat('ALGSTATUS_', $allergyNodeCntDisplay)" />
			</xsl:attribute>			
	...
	<xsl:choose>
	<xsl:when test="$summaryMode = 'active'">
	 <xsl:if test="$totalActiveAllergyNodes > $allergyNodeCnt">
      <xsl:call-template name="AllergySummary">
        ...
        <xsl:with-param name="allergyNodeCntDisplay" select="$allergyNodeCnt + 1"/>
        <xsl:with-param name="summaryMode" select="'active'"/>
      </xsl:call-template>
    </xsl:if>
    </xsl:when>
    <!-- <xsl:when test="$summaryMode = 'inactive'">
     <xsl:if test="$totalInactiveAllergyNodes > $allergyNodeCnt">
      <xsl:call-template name="AllergySummary">
      	<xsl:with-param name="allergyNode" select="$allergyNode"/>
        <xsl:with-param name="totalAllergyNodes" select="$totalInactiveAllergyNodes" />
        <xsl:with-param name="allergyNodeCnt" select="$allergyNodeCnt + 1" />
        <xsl:with-param name="allergyNodeCntDisplay" select="$totalActiveAllergyNodes + $allergyNodeCnt + 1"/>
        <xsl:with-param name="summaryMode" select="'inactive'"/>
      </xsl:call-template>
    </xsl:if>
    </xsl:when> -->
...
<xsl:template name="ProblemSummary" xmlns="urn:hl7-org:v3">
	...
	<xsl:param name="problemNodeCntDisplay" select="0"/>
	...
	<tr>
		<xsl:attribute name="ID">
			<xsl:value-of select="concat('PROBSUMMARY_', $problemNodeCntDisplay)" />
		</xsl:attribute>
		<td>
		...
		<td>
			<xsl:attribute name="ID">
				<xsl:value-of select="concat('PROBKIND_', $problemNodeCntDisplay)" />
			</xsl:attribute>	
			<xsl:value-of select="$problemPath/results/rows/row/title" />
		</td>
		<td>
			<xsl:attribute name="ID">
				<xsl:value-of select="concat('PROBSTATUS_', $problemNodeCntDisplay)" />
			</xsl:attribute>
	...
	<xsl:when test="$summaryMode = 'active'">
	 <xsl:if test="$totalActiveProblemNodes > $problemNodeCnt">
      <xsl:call-template name="ProblemSummary">
        ...
        <xsl:with-param name="problemNodeCntDisplay" select="$problemNodeCnt + 1"/>
        <xsl:with-param name="summaryMode" select="'active'"/>
      </xsl:call-template>
    </xsl:if>
    </xsl:when>
    <xsl:when test="$summaryMode = 'inactive'">
    <xsl:if test="$totalInactiveProblemNodes > $problemNodeCnt">
      <xsl:call-template name="ProblemSummary">
      	<xsl:with-param name="problemNode" select="$problemNode"/>
        <xsl:with-param name="totalProblemNodes" select="$totalInactiveProblemNodes" />
        <xsl:with-param name="problemNodeCnt" select="$problemNodeCnt + 1" />
        <xsl:with-param name="problemNodeCntDisplay" select="$totalActiveProblemNodes + $problemNodeCnt + 1"/>
        <xsl:with-param name="summaryMode" select="'inactive'"/>
      </xsl:call-template>
    </xsl:if>
    </xsl:when>
    <xsl:when test="$summaryMode = 'resolved'">
    <xsl:if test="$totalResolvedProblemNodes > $problemNodeCnt">
      <xsl:call-template name="ProblemSummary">
      	<xsl:with-param name="problemNode" select="$problemNode"/>
        <xsl:with-param name="totalProblemNodes" select="$totalResolvedProblemNodes" />
        <xsl:with-param name="problemNodeCnt" select="$problemNodeCnt + 1" />
        <xsl:with-param name="problemNodeCntDisplay" select="$totalActiveProblemNodes + $totalInactiveProblemNodes + $problemNodeCnt + 1"/>
        <xsl:with-param name="summaryMode" select="'resolved'"/>
      </xsl:call-template>
    </xsl:if>
    </xsl:when>
...
<xsl:template name="MedicationSummary" xmlns="urn:hl7-org:v3">
	...
	<xsl:variable name="medPath" select="document($medNode/child::node()[position()=$medNodeCnt]/drilldown)"/>
	<tr>
		<xsl:attribute name="ID">
		...
		<td>
			<xsl:if test="contains($medPath/results/rows/row/brand, '[')">
				<xsl:value-of select="substring-after(substring-before($medPath/results/rows/row/brand, ']'), '[')" />
			</xsl:if>
		</td>
		...
		<td>
			...
			<xsl:value-of select="$medPath/results/rows/row/status" /> 
...
<xsl:variable name="labresultPath" select="document($labResultNode/child::node()[position()=$labResultNodeCnt]/drilldown)"/>	
	...
	<!-- 	<tr>
		...
		<tr>	
			<xsl:attribute name="ID">
				<xsl:value-of select="concat('LABRESULT_', $labResultNodeCnt)" />
			</xsl:attribute>		
			...
			<xsl:value-of select="$labResultNode/child::node()[position()=$labResultNodeCnt]/Units" />
			<xsl:value-of select="$labresultPath/results/rows/row/nonrangeresult" />
			</td><!-- value 13.2 units -->			
...
<xsl:template name="ProcedureSummary" xmlns="urn:hl7-org:v3">
	...
	<xsl:variable name="procedurePath" select="document($procNode/child::node()[position()=$procNodeCnt]/drilldown)"/>	
		...
		<tr>
			<xsl:attribute name="ID">
				<xsl:value-of select="concat('PROC_', $procNodeCnt)" />
			</xsl:attribute>
...
<xsl:template name="AllergyDetailBlank" xmlns="urn:hl7-org:v3">
	<entry typeCode="DRIV">
			...
			<entryRelationship typeCode="SUBJ" inversionInd="false">
				<observation classCode="OBS" moodCode="EVN">
					...
					<text>
						 <reference>
							<xsl:attribute name="value">
							<!--	<xsl:value-of select="concat('#ALGSUMMARY_', $allergyDetailNodeCntDisplay)" />-->
							</xsl:attribute>
						</reference> 
					</text>
					<statusCode>
					...
					<value><!--  codeSystem="2.16.840.1.113883.6.88" -->
						...
						<originalText>
							 <reference>
								<xsl:attribute name="value">
								<!--	<xsl:value-of select="concat('#ALGSUB_', $allergyDetailNodeCntDisplay)" />-->
								</xsl:attribute>
							</reference> 
						</originalText>
					</value>
...
<xsl:template name="AllergyDetail" xmlns="urn:hl7-org:v3">
	...
	<xsl:param name="allergyDetailNodeCntDisplay" select="0"/>
	<xsl:param name="totalAllergyNodes" select="0" />
	<xsl:param name="summaryMode"/>
	<entry typeCode="DRIV">
		<act classCode="ACT" moodCode="EVN">		
				...
				<observation classCode="OBS" moodCode="EVN">
					...
					<value codeSystem="2.16.840.1.113883.6.88">
						<xsl:attribute name="xsi:type">CD</xsl:attribute>						
							...
							<xsl:if test="string-length($allergyPath/results/rows/row/codeSystemName) > 0">
	...
	<xsl:if test="$totalActiveAllergyNodes > $allergyDetailNodeCnt">
      <xsl:call-template name="AllergyDetail">
        ...
        <xsl:with-param name="allergyDetailNodeCntDisplay" select="$allergyDetailNodeCnt +1"/>
        <xsl:with-param name="summaryMode" select="'active'"/>
      </xsl:call-template>
    </xsl:if>
    </xsl:when>
    <!-- <xsl:when test="$summaryMode = 'inactive'">
    <xsl:if test="$totalInactiveAllergyNodes > $allergyDetailNodeCnt">
      <xsl:call-template name="AllergyDetail">
      	<xsl:with-param name="allergyDetailNode" select="$allergyDetailNode"/>
        <xsl:with-param name="totalAllergyNodes" select="$totalInactiveAllergyNodes" />
        <xsl:with-param name="allergyDetailNodeCnt" select="$allergyDetailNodeCnt + 1" />
        <xsl:with-param name="allergyDetailNodeCntDisplay" select="$totalActiveAllergyNodes + $allergyDetailNodeCnt +1"/>
        <xsl:with-param name="summaryMode" select="'inactive'"/>
      </xsl:call-template>
    </xsl:if>
    </xsl:when> -->
...
<xsl:template name="ProblemDetailBlank" xmlns="urn:hl7-org:v3">
	<entry typeCode="DRIV">
		<act classCode="ACT" moodCode="EVN">
			...
			<entryRelationship typeCode="SUBJ" inversionInd="false">
				<observation classCode="OBS" moodCode="EVN">
					...
					<text>
						<reference>
						 <xsl:attribute name="value">
								<!--	<xsl:value-of select="concat('#PROBSUMMARY_', $problemDetailNodeCntDisplay)" />-->
							</xsl:attribute> 
						</reference>
					</text>
					<statusCode>
...
<xsl:template name="ProblemDetail" xmlns="urn:hl7-org:v3">
		...
		<xsl:param name="problemDetailNodeCntDisplay" select="0"/>
		<xsl:param name="totalProblemNodes" select="0" />
		<xsl:param name="summaryMode"/>
	<entry typeCode="DRIV">
		<act classCode="ACT" moodCode="EVN">
	...
	<xsl:if test="$totalActiveProblemNodes > $problemDetailNodeCnt">
      <xsl:call-template name="ProblemDetail">
        ...
        <xsl:with-param name="problemDetailNodeCntDisplay" select="$problemDetailNodeCnt + 1"/>
        <xsl:with-param name="summaryMode" select="'active'"/>
      </xsl:call-template>
    </xsl:if>
    </xsl:when>
    <xsl:when test="$summaryMode = 'inactive'">
    <xsl:if test="$totalInactiveProblemNodes > $problemDetailNodeCnt">
      <xsl:call-template name="ProblemDetail">
      	<xsl:with-param name="problemDetailNode" select="$problemDetailNode"/>
        <xsl:with-param name="totalProblemNodes" select="$totalInactiveProblemNodes" />
        <xsl:with-param name="problemDetailNodeCnt" select="$problemDetailNodeCnt + 1" />
        <xsl:with-param name="problemDetailNodeCntDisplay" select="$totalActiveProblemNodes + $problemDetailNodeCnt +1"/>
        <xsl:with-param name="summaryMode" select="'inactive'"/>
      </xsl:call-template>
    </xsl:if>
    </xsl:when>
    <xsl:when test="$summaryMode = 'resolved'">
    <xsl:if test="$totalResolvedProblemNodes > $problemDetailNodeCnt">
      <xsl:call-template name="ProblemDetail">
      	<xsl:with-param name="problemDetailNode" select="$problemDetailNode"/>
        <xsl:with-param name="totalProblemNodes" select="$totalResolvedProblemNodes" />
        <xsl:with-param name="problemDetailNodeCnt" select="$problemDetailNodeCnt + 1" />
        <xsl:with-param name="problemDetailNodeCntDisplay" select="$totalActiveProblemNodes + $totalInactiveProblemNodes + $problemDetailNodeCnt +1"/>
        <xsl:with-param name="summaryMode" select="'resolved'"/>
      </xsl:call-template>
    </xsl:if>
    </xsl:when>
...
<xsl:template name="MedicationDetailBlank" xmlns="urn:hl7-org:v3">
	<entry typeCode="DRIV">
				...
				<period>
					<xsl:choose>
						<xsl:when test="string-length($medDetailNode/child::node()[position()=$medDetailNodeCnt]/frequency) > 0"> 
							<xsl:choose>
								<xsl:when test="number(substring($medDetailNode/child::node()[position()=$medDetailNodeCnt]/frequency,1,1))">
								...
								<xsl:when test="not(number(substring($medDetailNode/child::node()[position()=$medDetailNodeCnt]/frequency,1,1)))">
									<xsl:if test="contains(substring($medDetailNode/child::node()[position()=$medDetailNodeCnt]/frequency,2,1), '=')">
										<xsl:attribute name="unit"><xsl:value-of select="substring($medDetailNode/child::node()[position()=$medDetailNodeCnt]/frequency,1,1)" /></xsl:attribute>
									</xsl:if>
									<xsl:if test="not(contains(substring($medDetailNode/child::node()[position()=$medDetailNodeCnt]/frequency,2,1), '='))">
										<xsl:attribute name="unit"><xsl:value-of select="substring($medDetailNode/child::node()[position()=$medDetailNodeCnt]/frequency,1,2)" /></xsl:attribute>
									</xsl:if>
								</xsl:when>
							</xsl:choose>	
						</xsl:when>						
...
<xsl:template name="ProcedureDetailBlank" xmlns="urn:hl7-org:v3">
		<entry typeCode="DRIV">
		<procedure classCode="PROC" moodCode="EVN">
			...
			<code code="20109005" codeSystem="2.16.840.1.113883.6.96" codeSystemName="SNOMED CT">
				<originalText>
					<reference>
						<xsl:attribute name="value">
							<!--<xsl:value-of select="concat('#PROC_', $procDetailNodeCnt)" />-->
						</xsl:attribute> 
					</reference>
				</originalText>
			</code>
...
<xsl:template name="LabResultDetailBlank" xmlns="urn:hl7-org:v3">
	<entry typeCode="DRIV">
			...
			<component>
				...
				<observation classCode="OBS" moodCode="EVN">
					...
					<text>
						<reference>
							 <xsl:attribute name="value">
								<!--<xsl:value-of select="concat('#LABRESULT_', $labresultDetailNodeCnt)" />-->
							</xsl:attribute> 
						</reference>
					</text>
					<statusCode>
			...
			<component>
				...
				<observation classCode="OBS" moodCode="EVN">
					...
					<value>					
						...
						<xsl:choose>	
							<xsl:when test="$labresultDetailNode/child::node()[position()=$labresultDetailNodeCnt]/Value != ''">
							...
							<xsl:when test="$labresultDetailPath/results/rows/row/nonrangeresult != ''">
								<xsl:attribute name="xsi:type">ST</xsl:attribute>							
								<xsl:value-of select="$labresultDetailPath/results/rows/row/nonrangeresult"/><!-- 140-->
							</xsl:when>
							<xsl:otherwise>
								<xsl:attribute name="xsi:type">ST</xsl:attribute>	
								<xsl:attribute name="nullFlavor">UNK</xsl:attribute>
						...
						<xsl:choose>	
							<xsl:when test="$labresultDetailNode/child::node()[position()=$labresultDetailNodeCnt]/Units != ''">							
