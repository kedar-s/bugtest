...
public class ComputeReferenceRange extends ComputeBase{
	...
	public void compute() throws Exception {
	    ...
	    if (null != trimAct)
		{
		    for (ActRelationship rel : trimAct.getRelationships())
		    {
		    	if(rel.getName().equals("labresult"))
		    	{
		    		
		    if (null != rel.getAct().getObservation().getValues().get(0).getPQ())
			{
			    ...
			    for (ActRelationship lRel : rel.getAct().getRelationships())
				{
				...
				{
				    selectedRelationship = rel.getAct().getRelationships().get(0);
				}
			    ...
			    if (null != selectedRelationship)
				{
				    ...
				    boolean hasLowValue = (null != selectedIVLPQ.getLow());
				    boolean hasHighValue = (null != selectedIVLPQ.getHigh());
