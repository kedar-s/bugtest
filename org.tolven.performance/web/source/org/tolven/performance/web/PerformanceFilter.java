 ...
 * @author Scott DuVall
 */
@WebFilter(urlPatterns = { "*.ajax", "*.ajaxc", "*.ajaxi", "*.jsf", "*.jsp" })
    ...
    public PerformanceDataDAO getPerformanceDataBean() throws ServletException {
        try {
            if (perfDAO == null) {
                InitialContext ctx = new InitialContext();
                //TODO Injection does not work for JBoss (v4.0.4GA) web tier (tomcat), but does for GlassFish
                perfDAO = (PerformanceDataDAO) ctx.lookup("java:app/tolvenEJB/PerformanceDataDAOBean!org.tolven.core.bean.TolvenPropertiesLocal");
            }
        } catch (NamingException e) {
