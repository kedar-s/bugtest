...
public class ProcessCCR extends AppEvalAdaptor implements ProcessCCRLocal {
	...
	private static Logger logger = Logger.getLogger(ProcessCCR.class);
		...
		try {
			if (message instanceof TolvenMessage || 
				...
				if (CCRns.equals(tm.getXmlNS())) {
					...
					// Run the rules now
					runRules( );
					logger.info( "Processed CCR document " + getDocument().getId() + " for account: " + tm.getAccountId());
				}
			}
		} catch (Exception e) {
	...
	protected void loadWorkingMemory( StatefulSession workingMemory ) throws Exception {
		ccr = (ContinuityOfCareRecordEx) getCCRMarshaller().unmarshal(tm.getXmlNS(), new ByteArrayInputStream(tm.getPayload()));
	...
	public void processPatient(StatefulSession workingMemory) throws JAXBException, ParseException, CCRException,
		...
		if (mdPatient == null) {
			...
			if (dob != null && dob.getExactDateTime() != null) {
				...
				// ts.setValue(HL7DateFormatUtility.formatHL7TSFormatL8Date(dob.getDateValue()));
