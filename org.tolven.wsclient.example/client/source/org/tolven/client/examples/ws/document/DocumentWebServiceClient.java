...
public class DocumentWebServiceClient {
     ...
     * 
     * java -Djavax.net.ssl.keyStore=keyStoreFile -Djavax.net.ssl.trustStore=trustStoreFile org.tolven.client.examples.ws.document.DocumentWebServiceClient http://dev.able.com:8080/ws/document?wsdl filename accountIdNumber userIdNumber
    ...
    public static void main(String[] args) throws Exception {
        if (args.length != 3) {
            System.out.println("Usage: java -Djavax.net.ssl.keyStore=keyStoreFile -Djavax.net.ssl.keyStorePassword=tolven -Djavax.net.ssl.trustStore=trustStoreFile org.tolven.client.examples.ws.document.DocumentWebServiceClient https://dev.able.com:8443/ws/document?wsdl filename accountIdNumber");
            System.exit(1);
        }
        String className = System.getProperty("org.tolven.defaultHostnameVerifierClass");
        if (className != null) {
            Class<?> clazz = Class.forName(className);
            HttpsURLConnection.setDefaultHostnameVerifier((HostnameVerifier) clazz.newInstance());
        }
        String wsdl = args[0];
        ...
        String xml = DocumentWebServiceClient.getXML(new File(filename));
        Long result = port.processDocument(xml.getBytes(), CCRns, accountId);
        port.logout();
