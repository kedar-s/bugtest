...
public class ChartAction extends TolvenAction {
	...
	protected MenuLocal getMenuBean() {
        if (menuBean == null) {
            try {
            	menuBean = (MenuLocal) getContext().lookup("tolven/MenuBean/local");
            } catch (NamingException ex) {
                throw new RuntimeException("Could not lookup tolven/MenuBean/local", ex);
            }
        }
        return menuBean;
    }
	...
	private List<Map<String,Object>> findMenuDataList(String path, String contextPath, String condition, AccountUser accountUser) throws ParseException {
		List<Map<String, Object>> list = null;				
		MenuQueryControl ctrl = new MenuQueryControl();
		DateFormat df = new SimpleDateFormat("MM/dd/yy");
		String splitChar = ":";
		MenuStructure ms = getMenuBean().findMenuStructure(accountUser.getAccount().getId(), path );
		...
		list =  getMenuBean().findMenuDataByColumns(ctrl).getResults();
		return list;
	}
	...
	public XYDataset createDataset(int type, int gender, long age, Date dob) {
		try{
			...
			// For Height Chart
			if (type == 1) {
				List<Map<String, Object>> htList = findMenuDataList("echr:patient:observations:active", getRequestParameter("element").toString(),	"TestFilter=Height", TolvenRequest.getInstance().getAccountUser());
				...
				if(htList != null){
					...
					for (Map<String, Object> ht : htList) {
						...
						String months = new Integer(months(Integer.parseInt(dfm.format(dfcur.parse(ht.get("Date").toString()))), 
											Integer.parseInt(dfy.format(dfcur.parse(ht.get("Date").toString()))), 
											Integer.parseInt(dfm.format(dob)), 
											Integer.parseInt(dfy.format(dob)))).toString();
						double cm = 2.54 * (12*feet + inch);
						double tot = 0;
						double totCnt = 0;
						// month less that 20 years
					...
					for (Map.Entry<String, Object> entry : totheight.entrySet()) {
					    System.out.println("Key = " + entry.getKey() + ", Value = " + (Double.parseDouble(entry.getValue().toString())/
					    		Double.parseDouble(cntheight.get(entry.getKey().toString()).toString())));
					    htSeries.add(Double.parseDouble(entry.getKey().toString()), 
			...
			// For Weight Chart
				...
				List<Map<String, Object>> wtList = findMenuDataList("echr:patient:observations:active", getRequestParameter("element").toString(),	"TestFilter=Weight", TolvenRequest.getInstance().getAccountUser());
				if(wtList != null){
					...
					for (Map<String, Object> wt : wtList) {
						if (wt.get("Value")!=null && !wt.get("Value").equals("")) {
							...
							if (wt.get("Units").toString().equals("lb"))
								kg = kg * 0.45;
							String months = new Integer(months(Integer.parseInt(dfm.format(dfcur.parse(wt.get("Date").toString()))), 
												Integer.parseInt(dfy.format(dfcur.parse(wt.get("Date").toString()))), 
												Integer.parseInt(dfm.format(dob)), 
													Integer.parseInt(dfy.format(dob)))).toString();
							double tot = 0;
							double totCnt = 0;
					...
					for (Map.Entry<String, Object> entry : totweight.entrySet()) {
					    System.out.println("Key = " + entry.getKey() + ", Value = " + (Double.parseDouble(entry.getValue().toString())/
					    		Double.parseDouble(cntweight.get(entry.getKey().toString()).toString())));
					    htSeries.add(Double.parseDouble(entry.getKey().toString()), 
					    		Double.parseDouble(entry.getValue().toString())/
					    		Double.parseDouble(cntweight.get(entry.getKey().toString()).toString()));
					}
	...
	public JFreeChart createChart(int type) {
		...
		MenuData patMD = getMenuBean().findMenuDataItem(patientId);
		int age = 0;
		int gender = 1;
		Date dob = null;
		if (patMD != null) {
