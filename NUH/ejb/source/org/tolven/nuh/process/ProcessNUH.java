 ...
 *
 * @author <your name>
 * @version $Id: ProcessNUH.java,v 1.3 2009/06/29 06:29:49 jchurin Exp $
 */  
...
public class ProcessNUH extends AppEvalAdaptor {
	...
	// XRayReport fields
	private final XPathExpression examPath = path("exam_c"); // D0340R, D1321N, 0051700 procedure code
	private final XPathExpression statusPath = path("status_c"); // TRANS, APRV "approved"
	private final XPathExpression priorityPath = path("priority_c"); // S, R
	private final XPathExpression filmCountPath = path("no_films_n"); // 0
	private final XPathExpression radiologistPath = path("radiolg_c"); // NDITYL, NDILLN, NDI LLN
	private final XPathExpression radiologist2Path = path("radiolg2_c"); // 
	private final XPathExpression radiographerPath = path("radiogr_c"); // NDIKMC, NDIRWC
	private final XPathExpression patLocationPath = path("PatLocation"); // NW76, NCKCOL, NCA\T\E, NW55
	private final XPathExpression ludatePath = path("ludate_d"); // 
	private final XPathExpression cancelDatePath = path("proc_cancel_d"); // only 2000-01-01T00:00:00
	...
	protected MenuData bindToPatient( Node patient ) {
		try {
			...
			// We'll use HRN in subsequent xpath expressions to find elements for a specific patient
			...
			// Construct a small document with this xml node in it
			...
			// Use metadata to populate patient placeholder
			ee.pushContext();
	...
	protected MenuData bindToEncounter( Node visit, MenuData mdPatient ) {
		try {
			...
			// We'll use acct_n in subsequent xpath expressions to find elements for a specific encounter
			...
			if (mdEncounters.size() > 0) {
				...
				// Add visit number (acct) to MenuData item
			...
			ee.addVariable("nuh", xpathMap);
			// Now pull attributes into menuData using metadata instructions
			menuBean.populateMenuData(ee, mdEncounter);
			ee.popContext();
			persistMenuData(mdEncounter);
			// Add attending and admitting if present.
			...
			// Put medical service in location (not actually the same thing)
	...
	protected void loadMedCert( MenuData mdEncounter, List<Node> documentNodes ) throws Exception {
		// checkin id is already set, so just get the results for this order.
...
//			String account = accountPath.evaluate(visit);
			...
			if (mdXRayOrders.size() == 1) {
				...
				// Add unique XRay number to MenuData item
				mdXRayOrder.addPlaceholderID("xrayorderid", checkinId, "nuh");
			}
			createDocument(mdXRayOrder);
			// Give them a nice XML document to look at
			List<Node> documentNodes = new ArrayList<Node>(1);
			documentNodes.add(xrayOrder);
			// Populate placeholder via column metadata
			...
			// Take care of results that go with this order
			ee.popContext();
			return mdXRayOrder;
		} catch (Exception e) {
	...
	protected void loadXRayResults( MenuData mdXRayOrder, List<Node> documentNodes ) throws Exception {
		// Now do result(s) for this order
...
//			String ludate = ludatePath.evaluate(xrayReport);
...
//			mdXRayResult.setField("cancelDate", cancelDate.toString());
	...
	protected MenuData bindToLabOrder(Node labOrder, MenuData mdPatient ) throws Exception {
		...
		MenuStructure msOrder = menuBean.findMenuStructure(getAccount(), ":patient:request");
//		String account = accountPath.evaluate(visit);
		// Get the id of the order in order to find results below
		...
		if (mdLabOrders.size() == 1) {
			...
			// Add unique lab number to MenuData item
...
//		logger.info( "Document created, id: " + docBase.getId() + " Account: " + docBase.getAccount().getId());
	...
	protected void 	addLabResultFields( Node labResult, MenuData mdLabResult, MenuData mdLabOrder) throws Exception {
		// Effective time of the result is receive time of the order
