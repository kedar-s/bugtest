...
public class EARAssembler extends TolvenCommandPlugin {
    ...
    public static final String EXNPT_DATASOURCE = "datasource";
    @Deprecated
    ...
    public static final String EXNPT_PU = "persistence-unit";
    ...
    public static final String PARAM_CONTAINS_ENTITIES = "contains-entities";
    public static final String PARAM_DATABASE_CODE = "database-code";
    ...
    protected void addEJBModuleTemplates(PluginDescriptor pd, List<String> entityJars, File localDestDir, XMLStreamWriter writer) throws XMLStreamException, IOException {
        writer.writeStartElement("xsl:template");
        writer.writeAttribute("name", "addEJBModules");
        writer.writeCharacters("\n");
        assembleEJBModuleAdaptors(pd, entityJars, writer);
        assembleEJBModuleProducts(pd, localDestDir, writer);
        assembleEJBModuleProductAdaptors(pd, entityJars, localDestDir, writer);
        writer.writeEndElement();
        writer.writeCharacters("\n");
    }
    ...
    protected void addJavaLibraryTemplates(PluginDescriptor pd, List<String> entityJars, File localDestDir) throws IOException {
        ...
        assembleLibProductAdaptors(pd, entityJars, localDestDir);
    ...
    private void addPersistenceUnit(List<String> entityJars, XMLStreamWriter xmlStreamWriter) throws XMLStreamException {
        ExtensionPoint abstractPUExnPt = getAbstractEARPluginDescriptor().getExtensionPoint(EXNPT_PU);
        for (Extension puExn : abstractPUExnPt.getConnectedExtensions()) {
            xmlStreamWriter.writeStartElement("persistence-unit");
            String name = puExn.getParameter("name").valueAsString();
            if (name == null || name.length() == 0) {
                throw new RuntimeException(puExn.getUniqueId() + "@name must have a value");
            }
            xmlStreamWriter.writeAttribute("name", name);
            String transactionType = puExn.getParameter("transaction-type").valueAsString();
            if (transactionType == null || transactionType.length() == 0) {
                throw new RuntimeException(puExn.getUniqueId() + "@transaction-type must have a value");
            }
            xmlStreamWriter.writeAttribute("transaction-type", transactionType);
            Parameter providerParam = puExn.getParameter("provider");
            if (providerParam != null) {
                xmlStreamWriter.writeStartElement("provider");
                String provider = providerParam.valueAsString();
                if (provider.trim().length() == 0) {
                    throw new RuntimeException(puExn.getUniqueId() + " requires a provider parameter value");
                }
                xmlStreamWriter.writeCharacters(provider);
                xmlStreamWriter.writeEndElement();
                xmlStreamWriter.writeCharacters("\n");
            }
            String jtaDataSource = puExn.getParameter("jta-data-source").valueAsString();
            if (jtaDataSource == null || jtaDataSource.length() == 0) {
                throw new RuntimeException(puExn.getUniqueId() + "@jta-data-source must have a value");
            }
            xmlStreamWriter.writeCharacters("\n");
            xmlStreamWriter.writeStartElement("jta-data-source");
            if (jtaDataSource.trim().length() == 0) {
                throw new RuntimeException(puExn.getUniqueId() + " requires a jta-data-source parameter value");
            }
            xmlStreamWriter.writeCharacters(jtaDataSource);
            xmlStreamWriter.writeEndElement();
            xmlStreamWriter.writeCharacters("\n");
            for (String entityJar : entityJars) {
                xmlStreamWriter.writeStartElement("jar-file");
                xmlStreamWriter.writeCharacters(entityJar);
                xmlStreamWriter.writeEndElement();
                xmlStreamWriter.writeCharacters("\n");
            }
            List<Parameter> propertyParams = new ArrayList<Parameter>();
            propertyParams.addAll(puExn.getParameters("property"));
            if (!propertyParams.isEmpty()) {
                PluginDescriptor pd = puExn.getDeclaringPluginDescriptor();
                xmlStreamWriter.writeStartElement("properties");
                for (Parameter propertyParam : propertyParams) {
                    xmlStreamWriter.writeStartElement("property");
                    String propertyName = propertyParam.getSubParameter("name").valueAsString();
                    if (propertyName.trim().length() == 0) {
                        throw new RuntimeException(puExn.getUniqueId() + " requires a name parameter value");
                    }
                    xmlStreamWriter.writeAttribute("name", propertyName);
                    String propertyValue = evaluate("#{pluginProperty['" + propertyName + "']}", pd);
                    if(propertyValue == null) {
                        propertyValue = propertyParam.getSubParameter("value").valueAsString();
                        if (propertyValue.trim().length() == 0) {
                            throw new RuntimeException(puExn.getUniqueId() + " requires a value parameter value");
                        }
                    }
    ...
    private void assembleDefaultPerstenceUnit(List<String> entityJars, File localDestDir) throws IOException {
        File dest = new File(localDestDir, "META-INF/persistence.xml");
        FileUtils.writeStringToFile(dest, getPersistenceXML(entityJars));
        File persistenceUnitJar = new File(localDestDir, "lib/default-persistence-unit.jar");
        TolvenJar.jarDir(localDestDir, persistenceUnitJar, "META-INF/persistence.xml", false);
        dest.delete();
    }
     ...
     * @throws XMLStreamException
     */
    protected void assembleEJBModuleAdaptors(PluginDescriptor pd, List<String> entityJars, XMLStreamWriter writer) throws IOException, XMLStreamException {
        /*
        ...
        if (exnPt != null) {
            for (Extension exn : exnPt.getConnectedExtensions()) {
                ...
                if (exn.getParameter("contains-entities") != null) {
                    entityJars.add("../" + ejbFilename);
                }
            }
        }
    }
     ...
     * @throws XMLStreamException
     */
    protected void assembleEJBModuleProductAdaptors(PluginDescriptor pd, List<String> entityJars, File localDestDir, XMLStreamWriter writer) throws IOException, XMLStreamException {
        ExtensionPoint exnPt = pd.getExtensionPoint(EXNPT_EJBMODULEPROD_ADPTR);
        if (exnPt != null) {
            ExtensionPoint dsExnPt = getAbstractEARPluginDescriptor().getExtensionPoint(EXNPT_DATASOURCE);
            Extension dsExn = dsExnPt.getConnectedExtensions().iterator().next();
            String dbCode = dsExn.getParameter(PARAM_DATABASE_CODE).valueAsString();
            for (Extension exn : exnPt.getConnectedExtensions()) {
                boolean assemble = false;
                Parameter dbCodeParam = exn.getParameter(PARAM_DATABASE_CODE);
                if (dbCodeParam == null) {
                    assemble = true;
                } else {
                    assemble = dbCode.equals(dbCodeParam.valueAsString());
                }
                if (assemble) {
                    for (File src : getAdaptorFiles(exn)) {
                        File dest = new File(localDestDir, "/" + src.getName());
     ...
     * @throws IOException
     */
    protected void assembleLibProductAdaptors(PluginDescriptor pd, List<String> entityJars, File localDestDir) throws IOException {
    ...
    protected void assemblerMetaInfAdaptors(PluginDescriptor pd, File localDestDir) {
        File dest = new File(localDestDir, "/META-INF");
        ExtensionPoint abstractExnPt = getAbstractEARPluginDescriptor().getExtensionPoint(EXNPT_METAINF_ADPTR);
        ...
        for (ExtensionPoint exnPt : abstractExnPt.getDescendants()) {
            if (exnPt.getDeclaringPluginDescriptor().getId().equals(pd.getId())) {
                for (Extension exn : exnPt.getConnectedExtensions()) {
                    ...
                    for (ExtensionPoint ejbExnPt : ejbPD.getExtensionPoints()) {
                        if (EXNPT_EJBMODULE_DECL.equals(ejbExnPt.getParentExtensionPointId())) {
                            declExnPts.add(ejbExnPt);
                        }
                    }
                    if (declExnPts.isEmpty()) {
                        throw new RuntimeException(ejbPD.getUniqueId() + " does not define a ejbModule-declaration extension-point");
                    }
                    /*
                    if (contextId == null) {
    ...
    private String getPersistenceXML(List<String> entityJars) {
        try {
            ...
            try {
                ...
                xmlStreamWriter.writeCharacters("\n");
                addPersistenceUnit(entityJars, xmlStreamWriter);
                xmlStreamWriter.writeEndElement();
                xmlStreamWriter.writeCharacters("\n");
    ...
    protected String getXSLT(PluginDescriptor pd, File localDestDir) throws XMLStreamException, IOException {
        ...
        try {
            ...
            List<String> entityJars = new ArrayList<String>();
            addJavaLibraryTemplates(pd, entityJars, localDestDir);
            addConnectorModuleTemplates(pd, localDestDir, writer);
            addEJBModuleTemplates(pd, entityJars, localDestDir, writer);
            addWARModuleTemplates(pd, localDestDir, writer);
            assembleDefaultPerstenceUnit(entityJars, localDestDir);
