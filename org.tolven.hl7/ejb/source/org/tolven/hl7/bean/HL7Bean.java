...
import org.tolven.hl7.LabReport;
...
public class HL7Bean implements HL7Local {
	...
	public boolean saveHL7Message(MenuData md) {
		try {
			...
			} else if (md != null && md.getPath().contains("labOrder")) {
				LabReport labReport = new LabReport(md, recevier);
				hl7Message.setXml01(labReport.createHL7().getBytes());
				em.persist(hl7Message);
			} else {
				boolean flag = Boolean.parseBoolean(md.getField("isReportable").toString());
				if (flag) {
					PHS phs = new PHS(md, recevier);
