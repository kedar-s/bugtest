 ...
 *
 * @author Joseph Isaac
 * @version $Id: FacesConfigAssembler.java 5562 2012-01-19 05:14:33Z joe.isaac $
 */
 ...
 *
 */
public class FacesConfigAssembler {
    ...
    public FacesConfigAssembler(TolvenCommandPlugin tolvenCommandPlugin) {
    ...
    protected boolean addApplicationChildren(PluginDescriptor pluginDescriptor, XMLStreamWriter xmlStreamWriter) throws XMLStreamException {
        ...
        for (Extension extension : getTolvenCommandPlugin().getExtensions(pluginDescriptor, "view-handler")) {
        ...
        for (Extension extension : getTolvenCommandPlugin().getExtensions(pluginDescriptor, "el-resolver")) {
        ...
        for (Extension extension : getTolvenCommandPlugin().getExtensions(pluginDescriptor, "localeConfig")) {
    ...
    protected boolean addLifeCycleTemplates(PluginDescriptor pluginDescriptor, XMLStreamWriter xmlStreamWriter) throws XMLStreamException {
        ...
        xmlStreamWriter.writeCharacters("\n");
        for (Extension extension : getTolvenCommandPlugin().getExtensions(pluginDescriptor, "lifecycle")) {
    ...
    public void execute(String[] args) throws Exception {
        ...
        if (!facesConfigXMLFile.exists()) {
            ...
            String templateFilename = getTolvenCommandPlugin().getDescriptor().getAttribute(ATTRIBUTE_TEMPLATE_FACESCONFIGXML).getValue();
            File templateFile = getTolvenCommandPlugin().getFilePath(templateFilename);
            if (!templateFile.exists()) {
                throw new RuntimeException("Could not locate: '" + templateFile.getPath() + "' in " + getTolvenCommandPlugin().getDescriptor().getId());
            }
        ...
        if (xslt == null) {
            ...
            File facesConfigXSLT = new File(getTolvenCommandPlugin().getPluginTmpDir(), "faces-configxml-xslt.xml");
    ...
    protected PluginDescriptor getFacesPluginDescriptor() {
        ExtensionPoint extensionPoint = getTolvenCommandPlugin().getDescriptor().getExtensionPoint(FACES_EXTENSIONPOINT);
        String parentPluginId = extensionPoint.getParentPluginId();
        PluginDescriptor pluginDescriptor = getTolvenCommandPlugin().getManager().getRegistry().getPluginDescriptor(parentPluginId);
    ...
    protected PluginDescriptor getTagLibPluginDescriptor() {
        ExtensionPoint extensionPoint = getTolvenCommandPlugin().getDescriptor().getExtensionPoint(TAGLIB_EXTENSIONPOINT);
        String parentPluginId = extensionPoint.getParentPluginId();
        PluginDescriptor pluginDescriptor = getTolvenCommandPlugin().getManager().getRegistry().getPluginDescriptor(parentPluginId);
        return pluginDescriptor;
    }
    ...
    protected String getXSLT(PluginDescriptor pluginDescriptor, String contextId) throws XMLStreamException {
