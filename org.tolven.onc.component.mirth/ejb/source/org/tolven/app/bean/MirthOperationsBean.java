...
public class MirthOperationsBean extends CreatorBean implements MirthOperationsLocal {
	...
	public void sendMessage(TrimEx trim) throws Exception{
		...
		try{
			...
			if(success)
				TolvenLogger.info("Sent Lab orders to Mirth("+host+") on port "+port, this.getClass());
			else
				throw(new Exception("Could not send Lab orders to Mirth("+host+") on port "+port));
		}
	...
	/**
	 * Update the Lab order status with the response from Mirth
	 * @author Syed
	 * @param accountUser
	 * @param mapList - the response message from Mirth
	 * added on 6/21/2010
	 */
	/*public void updateLabOrderStatus(AccountUser accountUser, List<Message> mapList){
		String docPath = mapList.get(0).getDocumentPath();
		Account account = accountUser.getAccount();
			MenuData updateMD = menuBean.findDefaultedMenuDataItem(account, docPath);
			try {
				updateTrimData(accountUser, "completed", mapList);
	...
	private void updateTrimData(AccountUser accountUser, String transitionName, List<Message> mapList,PrivateKey userPrivateKey) throws Exception{
		...
		StatusInfo sInfo = calculateTransition( trim, transitionName);
		...
		if (trim.getTolvenId(accountUser.getAccount().getId())!=null) {
			trim.addTolvenId(createTolvenId( mdPlaceholder, null, now, sInfo.getStatus(), accountUser.getUser().getLdapUID() ));
		}
		...
		for(Message msg : mapList){
			if(msg.isProcessed()){
				...
				relAct.setStatusCodeValue(sInfo.getStatus());
				
			}
		}
		for(ActRelationship actRel : orderList){
			relAct = (ActEx)actRel.getAct();
			if(!relAct.getStatusCodeValue().equalsIgnoreCase(sInfo.getStatus()))
				completed = false;
		}
		if(completed){
		...
		trim.addTolvenEventId(createTolvenId( mdEvent, null, now, sInfo.getStatus(), accountUser.getUser().getLdapUID() ));
	...
	public void updateDocument(TolvenUser user, Response resp, PrivateKey userPrivateKey){
		...
		for(Message msg : msgList){
			if(msgMap.containsKey(msg.getDocumentPath()))
				msgMap.get(msg.getDocumentPath()).add(msg);
			else{
		...
		for(Map.Entry<String, List<Message>> entry : msgMap.entrySet()){
			...
			for(AccountUser accountUser: accountUsers) {
				if (accountUser.getAccount().getId()== accountId){
					...
					break;
					//updateLabOrderStatus(accountUser,mapList);
				}
			}
			if(!isAccountUser)
				TolvenLogger.info("Mirth accountuser not found in account "+accountId, this.getClass());
		}
	}
