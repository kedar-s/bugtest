...
import org.tolven.web.util.HTMLOutputWriter;
/**
...
public class AjaxServlet extends HttpServlet {
    ...
    @EJB
    private ListDataLocal listDataLocal;
    @Override
	...
	private static void writeFormattedColumn( MSColumn msColumn, FormattedOutputWriter fow, String outputExpression,Object value, String charWidth ) {
		try {
			if(NumberUtils.isNumber(charWidth) && value != null && value.toString().length() >0){
				fow.writeExpression(outputExpression,value.toString(),Math.round ((msColumn.getWidth()*16) / Float.parseFloat(charWidth))-6);
			}else{
			...
			if (value==null) {
				...
				if (stringValue.length()==0) {
					...
					if (lMethodName != null && lMethodName.length() > 0)
					{
						...
						instantiateValue +=  "<a title='"+StringEscapeUtils.escapeHtml(md1.getString06())+"' href=\"#\" onclick=\"" + lMethodName + "('" + md1.getString02() + "','"+ element+ "','" + lMethodArgs + "')\">";
					}else{
					...
					//MenuData md1 = menuLocal.findMenuDataItem(id);
					instantiateValue =  "<a href=\"#\" onclick=\"" + lMethodName + "('" + id.toString()+ "','"+ element+ "','" + lMethodArgs + "')\">";
					fow.writeVerbatim(instantiateValue);
					fow.writeEscape( stringValue ); 
					fow.writeVerbatim("</a>");
				}else{
	...
	public static void writeMenuDataRow( FormattedOutputWriter fow, MDQueryResults mdQueryResults,
			Map<String, Object> rowData, MenuLocal menuLocal, HttpServletRequest req, String charWidth ) throws IOException {
		...
		for (MSColumn msColumn : msColumns) {
			if (msColumn.isVisible()) {
				...
				if (outputExpression!=null) {
					writeFormattedColumn(msColumn,fow, outputExpression,rowData.get(msColumn.getHeading()),charWidth);
				} else {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		String element = req.getParameter( "element");
		String charWidth = req.getParameter( "charWidth");
		try {
			// MenuData
			if (uri.endsWith("menuData.ajax")) {
    			...
    			if(path .getPath().startsWith("global:"))
    				ms =  menuLocal.findGlobalMenuStructure(path.getPath() );
    			else
    				ms =  menuLocal.findMenuStructure(accountUser, path.getPath() );
    			
    			if (sortCol != null) {
					// make sure that we send the column name as per the menu structure and
					// not as translated by the locale.
					List<MSColumn> sortedColumns = ms.getSortedColumns();					
					ResourceBundle bundle_app = TolvenRequest.getInstance().getResourceBundle();					
					for (int i = 0; i < sortedColumns.size(); i++ ) {
					   MSColumn aColumn = (MSColumn)sortedColumns.get(i);
					   String localeName = aColumn.getLocaleText(bundle_app);
					   if (localeName != null && localeName.equalsIgnoreCase(sortCol)) {
						   sortCol = aColumn.getHeading();
						   break;
					   }
					}
				}
				...
				if (gridType==null || gridType.length()==0 ||gridType.equalsIgnoreCase("undefined")) {
					gridType = "_default";
				}
				...
				MDQueryResults mdQueryResults = null;
				if(ms.getQuery() != null && (ms.getQuery().startsWith("query:") || ms.getQuery().startsWith("custom:"))){
					mdQueryResults = listDataLocal.findDataByColumns(ctrl );
				}else{
				 // Do the query using menuBean
					mdQueryResults = menuLocal.findMenuDataByColumns( ctrl );
				}
			    int line = 0;
			    // Prepare the list of columns we're going to populate
				for( Map<String, Object>  rowData : mdQueryResults.getResults()) {
			    	line++;
			    	writeMenuDataRow( fow, mdQueryResults, rowData, menuLocal, req,charWidth);
				}
				ee.popContext();
			}
