...
//		MenuData md = getMenuLocal().findMenuDataItem(getAccountId(), getElement());
		...
		if (ms==null)
			if(getTargetMenuPath().getPath().startsWith("global:"))
				ms = getMenuLocal().findGlobalMenuStructure(getTargetMenuPath().getPath() );
			else
				ms =  getMenuLocal().findMenuStructure( TolvenRequest.getInstance().getAccountUser(), getTargetMenuPath().getPath() );
		return ms;
	}
	
	public List<MSAction> getActions( ) {
		return getMenuLocal().getFilteredActions(getThisMenu(), TolvenRequest.getInstance().getAccountUser());
	}
...
//			TolvenLogger.info( getElement() + " children: " + childPaths, MenuAction.class);
	...
	public int getMemberLists() {
		String path =  "echr:patients:Set3List";
		int count = 0;
		try {
			// Get the MenuData item for this particular patient
			MenuData md = getMenuLocal().findMenuDataItem(TolvenRequest.getInstance().getAccount().getId(), getTargetMenuPath().getPathString());
			
			long x = getMenuLocal().findMenuStructureCount(md.getAccount(), path);
			if (x > 0) {
				MenuStructure msList = getMenuLocal().findMenuStructure(md.getAccount(), path);
			
				// Find the menu structure based on the user who is logged in and the full path to the desired list.
				List<MenuData> memberList = getMenuLocal().findReferencingMDs( md, msList );
	
				// Return the lists that the patient is a member of
				count = memberList.size();
			}
			
		} catch (IllegalArgumentException e) {
			TolvenLogger.warn("Unable to find the " + path + " list.  Check that it has been created using patient list designer.", MenuAction.class);
		}
...
//							TolvenLogger.info(msPath.getPathString() + " :- ADDED .. col number is null ", MenuAction.class);
	...
	public String getDefaultPathSuffix() {
		if(this.defaultPathSuffix == null){
			defaultPathSuffix = getAccessRoleLocal().findAccessibleChild(getThisMenu(), TolvenRequest.getInstance().getAccountUser());
		}
...
//			TolvenLogger.info( getElement() + " children: " + childPaths, MenuAction.class);
	...
	public String getSummaryMenuDataRows( ) {
		...
		for (MenuData md : mdList ) {
			...
			for (MSColumn col : cols) {
				sb.append("<td>");
				if(col.isReference()){
					sb.append("<a href=\"javascript:showPane('"+md.getReferencePath()+"',false)\">");
	...
	/**
	 * Created to get the ccd attachment to a document.
	 * @return
	 * @throws Exception
	 */
	public DocBase getAttachedCCDDocument()  throws Exception {
		List<DocAttachment> attachments = getDrilldownAttachments();
		try {
		   return attachments.get(attachments.size() -1).getAttachedDocument();
		} catch(Exception e) {   
		   return null;
		}
	}
	...
	public String getGridControl() throws IOException {
		if (grid==null) {
			...
			if(ctrl.getMenuStructure().getQuery() != null && (ctrl.getMenuStructure().getQuery().startsWith("query:") 
					|| ctrl.getMenuStructure().getQuery().startsWith("query:"))){
				menuDataCount = new Long( getListDataLocal().countListData(ctrl));
			}else{
				menuDataCount = new Long( getMenuLocal().countMenuData( ctrl ) );
			}
			grid = new GridBuilder(ctrl, menuLocal,menuDataCount);
...
//		TolvenLogger.info( "[getDocCCR] MD.id=" + md.getId(), MenuAction.class);
	...
	public ContinuityOfCareRecord getCcr( ) throws Exception {
		if (ccr==null && getDocCCR()!=null) {
			ccr = (ContinuityOfCareRecord) getCCRMarshaller().unmarshal(getDocCCR(), TolvenRequest.getInstance().getAccountUser(), getUserPrivateKey());
		}
		return ccr;
	}
...
//	        TolvenLogger.info("Preparing Graph", MenuAction.class);
	...
	public List<PlaceholderTransition> getEventTransitions() {
		try {
			MenuData md = getMenuDataItem( );
			return getMenuLocal().getEventTransitions(md);
			
		} catch (Exception e) {
			throw new RuntimeException("Unable to find placeholder transitions ", e);
		}
	...
	public List<String> getMenuAllowRoles() {
		if(menuAllowRoles == null){
			...
			if(menuToEdit != null && null != menuToEdit.getAccessRoles()){
				for(MSAccessRole role : menuToEdit.getAccessRoles()){
					if(role.getAllowed())
						menuAllowRoles.add( role.getRole());
				}
			}
		}
	...
	public List<String> getMenuDenyRoles() {
		if(menuDenyRoles == null){
			...
			if(menuToEdit != null && null != menuToEdit.getAccessRoles()){
				for(MSAccessRole role : menuToEdit.getAccessRoles()){
					if(!role.getAllowed())
						menuDenyRoles.add( role.getRole());
				}
			}
		}
	...
	public String updateMenuItem() {
		...
		Collection<MSAccessRole> roles = menuToEdit.getAccessRoles();
		Collection<MSAccessRole> newRoles = new ArrayList<MSAccessRole>();
		Collection<MSAccessRole> deletedRoles = new ArrayList<MSAccessRole>();
		//find changes to existing roles
		for(MSAccessRole role:roles){
			boolean deleteRole = true;
			for(String roleName:menuAllowRoles){
			...
			if(deleteRole){
				for(String roleName:menuDenyRoles){
					if(roleName.equals(role.getRole())){
		...
		//find new roles added
		Map<String,MSAccessRole> roleDict = menuToEdit.getAccessRoleDictonary();
		for(String roleName:menuAllowRoles){
			if(roleDict.get(roleName) == null){
				MSAccessRole role = new MSAccessRole();
				role.setAllowed(true);
				role.setRole(roleName);
				role.setMenuStructure(menuToEdit);
				newRoles.add(role);
			}				
		}
		for(String roleName:menuDenyRoles){
		...
		//purge & persist the roles
		getAccessRoleLocal().addAccessRoles(newRoles);
		getAccessRoleLocal().deleteAccessRoles(deletedRoles);
		//reset roles on menustructure
		roles.removeAll(deletedRoles);
		roles.addAll(newRoles);
		menuToEdit.setAccessRoles(roles);
	...
	public String getAttachmentType() {
		if (this.attachmentType==null) {
			Map<String,String> params = FacesContext.getCurrentInstance().getExternalContext().getRequestParameterMap();
			// Parse the element if specified
			this.attachmentType = params.get("attachmentType");
			if (this.attachmentType==null) {
				this.attachmentType = getParameterFromRequest("attachmentType");
			}
		}
