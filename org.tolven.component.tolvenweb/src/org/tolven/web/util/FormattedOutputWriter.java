...
public abstract class FormattedOutputWriter {
	...
	public void writeExpression( String expression,String value,int allowedLength ) throws IOException {
		int segmentBegin = -1;
		boolean escape = false;
		char e[] = expression.toCharArray();
		for (int c = 0; c < e.length; c++ ) {
			...
			if ('}'==e[c] && segmentBegin>=0) {
				String segment = expression.substring(segmentBegin, c+1);
				String result = (String) ee.evaluate(segment, String.class);
				if(result.equalsIgnoreCase(value) && result.length() > allowedLength){
					writeVerbatim("<span title='");
					writeEscape(result);
					writeVerbatim("'>");					
					writeEscape(result.substring(0,allowedLength)+"...");
					writeVerbatim("</span>");					
