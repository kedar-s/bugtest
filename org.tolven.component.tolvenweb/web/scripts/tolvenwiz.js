...
//Submit specified element (nothing is uploaded, which will already be done
function wizPHTransition( element, action ) {
    closeEventBubble();
 var myAjax = new Ajax.Request(
   'wizPHTransition.ajaxi',
   {
    method: 'get',
    parameters: 'element='+element+'&action='+action,
    onFailure: function(request) {displayError(request,element);},
 ...
 if (stepNo > 0) {
  var step = $(prefix+'step'+stepNo);
	...
	if (prefix.indexOf("echrpatients")!=-1 && stepNo==5) {  	
  		enablePHRCreationAndSharing(prefix);  // !!This applies to Patient Reminders section of New Patient Wizard
  	}
...
* Author Valsaraj
* Added on 06/15/2010 
*//* Copied from cchitWizardScripts, but attempting to remove.  !!This applies to Patient Reminders section of New Patient Wizard*/
 ...
 *//*Copied from cchitWizardScripts. Would like to remove. See reference in GridBuilder.java*/
...
//outputFormat - type of the output format to be used for creating the display content in the grid
function openPopup( msPath, placeholderid, formId, methodName, methodArgs,outputFormat){
 serialNo++;
// Tolven.Util.log( "Getting: " + contentName );
 $('downloadStatus').innerHTML="Get " + msPath + "...";
 ...
 // Update this block whenever a similar new wizard is added.
	new Ajax.Request(
	  ...
	  {
		method: 'get',
		parameters: "element="+msPath+"&gridId="+placeholderid+"&gridType="+outputFormat+"&methodArgs="+methodArgs+"&methodName="+methodName+"&formId="+formId,
...
//method to open a popup with list contents
//this is a duplicate method for openTemplate(). Since the name of the method
//openTemplate() can not be changed to support backward compatibility we should add a new method
function openListPopup(msPath, placeholderid, methodName, formId, index,outputFormat){
	openTemplate(msPath, placeholderid, methodName, formId, index,outputFormat);
}

//gridType - type of the output format to be used for creating the display content in the grid
function openTemplate(msPath, placeholderid, methodName, formId, index,outputFormat)
{
 ...
 openPopup( msPath, placeholderid, formId,  methodName, methodArgsStr,outputFormat);
}
...
function gridEvent(id,isDelete,srcGridId,destGridId,element,template){
 ...
 function showFavoritesGrid(contentName, placeholderid, methodName, args){
		...
		var index = params[1];
		var popupGrids;
		if($$("form#"+formId+" div#popupGrids").length >0)
			popupGrids = $$("form#"+formId+" div#popupGrids div");
		else{
			popupGrids = $(formId).hide();		
		}
		...
		if($(placeholderid) == null){
			...
			if($$("form#"+formId+" div#popupGrids").length >0){
				...
				$(formId).parentNode.appendChild(element);
			}
		}
		
		openTemplate(contentName, placeholderid, methodName, formId, index,methodName);
	 }
...
/*Method to trigger compute for adding placeholder associations*/
function addPlaceholderAssociation(placeholderPath, methodArgs){
 var lArgs = splitArguments(methodArgs);
 var formId = lArgs[0];
 var targetActIndex = lArgs[1];
 var targetComputeIndex = lArgs[2];
 var rootForm = $(formId);
 $(formId + ":addAssociationComputeEnable"+targetActIndex+targetComputeIndex).value = "true";
 $(formId + ":associationPlaceholderPath"+targetActIndex+targetComputeIndex).value = placeholderPath; 
 $(formId + ":addAssociationComputeAction"+targetActIndex+targetComputeIndex).value = "add"; 
 var wipNode = rootForm.parentNode;
 ajaxSubmit4(rootForm, wipNode.id, eval(targetActIndex));
}
/*Method to trigger compute for deleting placeholder associations*/
function deletePlaceholderAssociation(formId,targetActIndex,targetComputeIndex,associationIndex){
 var rootForm = $(formId);
 $(formId + ":addAssociationComputeEnable"+targetActIndex+targetComputeIndex).value = "true";
 $(formId + ":addAssociationComputePosition"+targetActIndex+targetComputeIndex).value = associationIndex;
 $(formId + ":addAssociationComputeAction"+targetActIndex+targetComputeIndex).value = "remove"; 
 var wipNode = rootForm.parentNode;
 ajaxSubmit4(rootForm, wipNode.id, eval(targetActIndex));
}

//function to instantiate a wizard from a dialog popup
function instantiateInPopup( templateId, context){
	closeAllPopGrids();
	instantiate(templateId, context);
}

//function to close all popupgrids in the application
function closeAllPopGrids(){
	var popupGrids =$$("div.popupgrid");
	for(i=0;i<popupGrids.length;i++){
		popupGrids[i].hide();	
	}
}