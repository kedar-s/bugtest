...
function ajaxUpdatingSubmit(action, rslt) {
	...
	function instantiateTrans( templateId, context, source, actStatus ) {
		var sourceParam;
		if (source == undefined) {
			sourceParam = "";
		} else {
		...
		var param = 'templateId='+templateId+'&context='+context+sourceParam+'&actStatus='+actStatus;
		var instAjax = new Ajax.Request(
			'instantiateTrans.ajaxi',
			{
				method: 'get',
				parameters: param,
...
//		Tolven.Util.log( "Server response (" + elements.length + "): " + req.responseText );
	...
	//			Tolven.Util.log( "Count: " + e.length + " Close pass one: " + e[x].id );
		...
		Element.remove( id );
		showPane(n);
		closeTabExtension(id);
	}
	function closeTabExtension(id){
...
function createGrid( menuPath,id, methodName, methodArgs,outputFormat ) {
	if(!id)
		id = menuPath;
	var root = $(id);
	var grid = $(id+'-grid');
...
//		root.setAttribute( 'gridPreset', grid.getAttribute( 'gridPreset' ));
	...
	liveGrids[id] = new Rico.LiveGrid( id+"-LG",
		...
		{	prefetchBuffer: true,
			...
			requestParameters: [{name: 'element', value: menuPath},
								...
								{name: 'gridType', value: outputFormat },
								{name: 'charWidth', value: charWidth}],
	...
	new Form.Element.Observer( $(id+"-filter"), 1, function(element, val) {checkUserInput(element, val, id, methodNameValue, methodArgsValue,menuPath,outputFormat);} );
	if( $(id).getAttribute('filterValue') != null && $(id).getAttribute('filterValue') != "" ) {
	  filterValueChange($(id + "-filter"), $(id).getAttribute('filterValue'), id, methodNameValue, methodArgsValue ,menuPath,outputFormat );
	}
...
function cui(element, value, id, methodName, methodArgs,menuPath,gridType){
	if( $( id + "-filter").value == value ){
	 filterValueChange(element, value, id ,methodName, methodArgs,menuPath,gridType);
	} else $(id).setAttribute('filterValue', $( id + "-filter").value );
}

function checkUserInput(element, value, id , methodName, methodArgs,menuPath,gridType){
	$(id).setAttribute('filterValue', value );
	setTimeout( "cui('" + element + "', '" + value + "', '" + id+  "', '" + methodName + "', '" +  methodArgs+"', '" +  menuPath + "','"+gridType+"')", 125);
}

function filterValueChange(element, val, id, methodName, methodArgs,menuPath,outputFormat) {
//Tolven.Util.log(val);
	if(!menuPath)
	...
	lg.setRequestParams( {name: 'element', value: menuPath}, {name: 'filter', value: val}, {name: 'methodName', value: methodName}, {name: 'methodArgs', value: methodArgs},{name: 'gridType',value:outputFormat} );
//	lg.resetContents();
...
function doSubstr(root,liveGrid){
	var _ths = root.getElementsByTagName("th");
	var headerRowWidth = {};
...
//		Tolven.Util.log(headerRowWidth[ i ]);
	...
	// calculate the charWidth on the browser		
	var charWidth = Math.round( $('testFontSize').offsetWidth / testChars.length );
	var gridViewPort = $(root.id+"-LG_viewport");
	var trs = gridViewPort.getElementsByTagName("tr");
	for( var i = 0; i < trs.length; i++){
		var tds = trs[i].getElementsByTagName("td");
		for(var j=0;j<tds.length;j++){
			//get the actual displyable text from livegrid.buffer.rowsContent
			if(!liveGrid.buffer.rowsContent[i][j])
				continue;
			var td_text = liveGrid.buffer.rowsContent[i][j];
			var allowedChars = Math.round ( headerRowWidth[ j ] / charWidth );
			if( td_text.length > allowedChars ){
				var displayText = tds[ j ].innerHTML;
				tds[ j ].title = td_text;
				//do the text truncation
				tds[ j ].innerHTML = displayText.replace(td_text,td_text.substring(0, allowedChars-5 ) + " ...");
			}			
		}			
	}
}
...
/*  This function is used in web/private/customizeMenuItem.xhtml

This is used to enable & disable he respective access control checkboxes for user roles when they changed.
For example for a role when the allow checkbox is checked the respective deny check box is disabled
and when the allow checkbox is unchecked the respective deny check box is enabled
*/
function changeRoleAccess(name,allowSection,denySection){
var params = name.split(':');
if($(name).checked){
	$(params[0]+":"+denySection+":"+params[2]).setAttribute("disabled","disabled");
}else{		
	$(params[0]+":"+denySection+":"+params[2]).removeAttribute("disabled");
}
}
/*This function is used in web/private/customizeMenuItem.xhtml
This is used to enable & disable he respective access control checkboxes for user roles when the page is loaded.
*/
function assembleRoleAccess(activeSection,inactiveSection){
var i =0;
while($(activeSection+":"+i) != null){
	if($(activeSection+":"+i).checked){
		$(inactiveSection+":"+i).setAttribute("disabled","disabled");
	}
	i++;
}
