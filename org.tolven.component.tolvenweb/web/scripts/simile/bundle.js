...
Timeline.create=function(elmt,bandInfos,orientation,unit,minDate,maxDate){
    return new Timeline._Impl(elmt,bandInfos,orientation,unit,minDate,maxDate);
};
...
Timeline.createBandInfo = function(params) {
    ...
    var loc = ("locale" in params) ? params.locale : Timeline.Platform.getDefaultLocale();
    return {   
        width:          params.width,
        eventSource:    eventSource,
...
Timeline._Impl=function(elmt,bandInfos,orientation,unit,minDate,maxDate){
    this._containerDiv = elmt;
    this.minDate = minDate;
    this.maxDate = maxDate;
    this._height = 0;
    this._scrollBar = null;
    this._scrollLabelsDiv = null;
...
Timeline._Impl.prototype.setHeight=function(height){
    this._height = height;
};
Timeline._Impl.prototype.getHeight=function(){
    return this._height;
};

Timeline._Impl.prototype.getScrollBar=function(){
...
Timeline._Band = function(timeline, bandInfo, index) {
    ...
    this.h_scroll_moving = false;
    this._dragging = false;
    this._changing = false;
    this._originalScrollSpeed = 5; // pixels
    this._scrollSpeed = this._originalScrollSpeed;
    this._onScrollListeners = [];
    this._eventPainters= new Array();
    ...
    //Timeline.DOM.registerEventWithObject(this._div,"dblclick",this,this._onDblClick);
     ...
     if(index == this._timeline._bandInfos.length-1){
         this._timeline._yearBand = this;
         this._scrollDiv = document.createElement("div");
         this._timeline._scrollBar = this._scrollDiv;

         this._scrollDiv.style.overflow="scroll";
         this._scrollDiv.style.position='relative';
         this._scrollDiv.style.left=this._timeline._containerDiv.offsetLeft;
         this._scrollDiv.style.width=this._timeline._containerDiv.offsetWidth+"px"; 
         this._scrollDiv.style.height='20px'; 
         this._scrollDivInner = document.createElement("div");
		 var _intervalPerUnit = 1;
		 
		 var _unit = this._timeline._bandInfos[0].etherPainter._unit;
		 var _msPerDay = 1000*60*60*24;
		 switch(_unit){
			 case 4:   // DAY
				this._timeline._scrollableUnits = (this._timeline.maxDate.getTime() - this._timeline.minDate.getTime())/_msPerDay;
				this._timeline.scrollableWidth = this._timeline._scrollableUnits * this._ether._pixelsPerInterval;
				break;
			 case 5:   // WEEK
				this._timeline._scrollableUnits = ((this._timeline.maxDate.getTime() - this._timeline.minDate.getTime())/_msPerDay)/7;
				this._timeline.scrollableWidth = this._timeline._scrollableUnits * this._ether._pixelsPerInterval;
				_msPerDay *= 7;
				break;
			 case 6:  // MONTH
				if(this._timeline.maxDate.getFullYear() != this._timeline.minDate.getFullYear()){
					this._timeline.scrollableWidth = (this._timeline.maxDate.getFullYear() - this._timeline.minDate.getFullYear()) *12 * this._ether._pixelsPerInterval;
				}
				else
					this._timeline.scrollableWidth = 12 * this._ether._pixelsPerInterval ;
				this._timeline._scrollableUnits = (this._timeline.maxDate.getFullYear() - this._timeline.minDate.getFullYear()) *12;
				_msPerDay *= 30;
				break;
			default:
				_intervalPerUnit = 1;
		 ...
		 this._timeline._pixelsPerInterval = this._ether._pixelsPerInterval;
	     this._scrollDivInner.style.width=this._timeline._containerDiv.offsetWidth+this._timeline.scrollableWidth+"px"; 
         this._scrollDivInner.style.height='10px'; 
         this._scrollDiv.appendChild(this._scrollDivInner);
         
		 this._timeline._mirrorBand = new Timeline.MirrorBand(this._timeline);
        
         this.showLoadingMsg= function(){
			 $('showLoadingMessage').show();
      	 };
          this._timeline._containerDiv.parentNode.appendChild(this._timeline._mirrorBand._containerDiv);
	      this._timeline._containerDiv.parentNode.appendChild(this._scrollDiv);
	      var yearsFrmMinDate = 0;
	      var monthsFrmMinDate = 0;
	      var currentDate = new Date();
	      // setting the scrollleft for the scrollbar
	      // 1) if the current is not in min and max dates range set the scrollleft to scrollableWidth
	      if(this._timeline.maxDate  < currentDate){
		 yearsFrmMinDate = this._timeline.maxDate.getFullYear() - this._timeline.minDate.getFullYear();
		 monthsFrmMinDate = this._timeline.maxDate.getMonth() - this._timeline.minDate.getMonth();
		 this._timeline.getScrollBar().scrollLeft= this._timeline.scrollableWidth // this._timeline.scrollableWidth;
	      }
	      //if the current date is in min and max dates range then find the left value= currentDate - mindate
	      // remember the maxdate is Dec-31 of the year of the last event
	      else if(this._timeline.maxDate  > currentDate){
		  yearsFrmMinDate = currentDate.getFullYear() - this._timeline.minDate.getFullYear();
		  monthsFrmMinDate = currentDate.getMonth() - this._timeline.minDate.getMonth();
		  this._timeline.getScrollBar().scrollLeft= Math.ceil((currentDate.getTime()- this._timeline.minDate.getTime())/(_msPerDay))*this._ether._pixelsPerInterval;
      }

    this.prevScrollLeft =this._scrollDiv.scrollLeft;

      this._timeline.yearBand = this;
      Timeline.DOM.registerEventWithObject(this._scrollDiv,"mousedown",this,this._onScrollDown);
      Timeline.DOM.registerEventWithObject(this._scrollDiv,"mousemove",this,this._onScrollMove);
      Timeline.DOM.registerEventWithObject(this._scrollDiv,"mouseup",this,this._onScrollUp);
      Timeline.DOM.registerEventWithObject(this._scrollDiv,"mouseout",this,this._onScrollOut);
      Timeline.DOM.registerEventWithObject(this._scrollDiv,"scroll",this,this._onScrollScroll);
  }
...
Timeline._Band.prototype.addEventPainter=function(eventPainter){
	this._eventPainters[this._eventPainters.length] = eventPainter;
};
...
Timeline._Band.prototype.getEventPainters=function(){
     return this._eventPainters;
};
...
Timeline._Band.prototype.setLocalVersion=function(version){
     this._localVersion = version;
};
Timeline._Band.prototype.getLocalVersion=function(){
     return this._localVersion;
};
Timeline._Band.prototype.setPath=function(pathStr){
     this._path = pathStr;
};
Timeline._Band.prototype.getPath=function(){
     return this._path;
};
Timeline._Band.prototype.getColor=function(){
     return this._color;
};
Timeline._Band.prototype.setColor=function(color){
     return this._color;
};
...
Timeline._Band.prototype.createLayerDivNew=function(zIndex,label){
    var div=this._timeline.getDocument().createElement("div");
    div.className = 'timeline-eventpainter-layer';
    div.style.zIndex=zIndex;
    
    var innerDiv=this._timeline.getDocument().createElement("div");
    innerDiv.className="timeline-eventpainter-layer-inner";
    if(Timeline.Platform.browser.isIE){
       innerDiv.style.cursor="move";
    }else{
       innerDiv.style.cursor="-moz-grab";
...
Timeline._Band.prototype._onScrollDown=function(innerFrame,evt,target){
	yearband = this._timeline.getBand(this._timeline.getBandCount()-1);
	yearband.h_scroll_moving = true;
};
Timeline._Band.prototype._onScrollUp=function(innerFrame,evt,target){
	yearband = this._timeline.getBand(this._timeline.getBandCount()-1);
	yearband.h_scroll_moving = false;
};
Timeline._Band.prototype._onScrollMove=function(innerFrame,evt,target){
	
};
Timeline._Band.prototype._onScrollOut=function(innerFrame,evt,target){
	yearband = this._timeline.getBand(this._timeline.getBandCount()-1);
};
Timeline._Band.prototype._onScrollScroll=function(innerFrame,evt,target){
	yearband = this._timeline.getBand(this._timeline.getBandCount()-1);
	if(!this._timeline._dragging){
		scrollLength = this._timeline.getScrollBar().scrollLeft - yearband.prevScrollLeft;
		yearband.prevScrollLeft = this._timeline.getScrollBar().scrollLeft;
		yearband._moveEther(-scrollLength);
		yearband._positionHighlight();
	}	
setBandsStyle(this._timeline,null,this._timeline._containerDiv.id);
};
...
Timeline._Band.prototype._onMouseMove=function(innerFrame,evt,target,scroll){
    if (this._dragging) {
        var diffX = evt.clientX - this._dragX;
        var diffY = evt.clientY - this._dragY;
        shiftDistance = this._timeline.isHorizontal()?diffX:diffY;
        yearband = this._timeline.getBand(this._timeline.getBandCount()-1)
        if((shiftDistance > 0 && this._timeline.getScrollBar().scrollLeft==0) ){
	      //yearband.setCenterVisibleDate(this._timeline.minDate);
	      return;
        }
        if(shiftDistance < 0 && this._timeline.getScrollBar().scrollLeft ==this._timeline.scrollableWidth){
	    //yearband.setCenterVisibleDate(this._timeline.maxDate);
	    return;
        }
        this._dragX = evt.clientX;
        this._dragY = evt.clientY;
        if(this._dragging){
	   timelinescroll = true;
	   this._timeline.getScrollBar().scrollLeft += (-shiftDistance);
	   timelinescroll = false;
	   this._timeline.getBand(this._timeline.getBandCount()-1).prevScrollLeft = this._timeline.getScrollBar().scrollLeft;
        }
        this._moveEther(shiftDistance);
        this._positionHighlight();
        setBandsStyle(this._timeline,null,this._timeline._containerDiv.id);
    }
};
...
Timeline._Band.prototype._paintEvents = function() {
	for(var i=0;i<this._eventPainters.length;i++)
		this._eventPainters[i].paint();
//this._eventPainter.paint();
};

Timeline._Band.prototype._softPaintEvents = function() {
	for(var i=0;i<this._eventPainters.length;i++)
		this._eventPainters[i].softPaint();
...
Timeline.EventIndex.prototype.elementAt = function(index) {
    return this._events.elementAt(index);
};
...
Timeline.Debug.exception = function(e) {
    Tolven.Util.log("Caught exception: " + (Timeline.Platform.isIE ? e.message : e));
};
...
Timeline.Graphics.createMessageBubble = function(doc) {
    ...
    containerDiv.id="showLoadingMessage";
    if (Timeline.Graphics.pngIsTranslucent) {
...
Timeline.GregorianEtherPainter.prototype.initialize = function(band, timeline) {
    ...
    //this._backgroundLayer.style.background=this._color;
...
Timeline.GregorianEtherPainter.prototype.paint = function() {
    ...
    this._lineLayer = this._band.createLayerDiv(2);
...
Timeline.HotZoneGregorianEtherPainter.prototype.initialize = function(band, timeline) {
    ...
    this._backgroundLayer = band.createLayerDiv(0);
    this._backgroundLayer.setAttribute("name","ether-background");
    this._backgroundLayer.style.background=this._theme.ether.backgroundColors[band.getIndex()]|| '#FFF';
...
Timeline.EtherHighlight = function(timeline, band, theme, backgroundLayer) {
    ...
    this._createHighlightDiv = function() {
        if (this._highlightDiv == null) {
            ...
            this._highlightDiv.style.background=band.getColor();
...
Timeline.GregorianDateLabeller.getMonthName = function(month, locale) {
    var mtName;
    try{
	mtName = Timeline.GregorianDateLabeller.monthNames[locale][month];
    }catch(err){
	// if not supported, use default
	mtName = Timeline.GregorianDateLabeller.monthNames["en"][month];
    }
...
Timeline.GregorianDateLabeller.prototype.defaultLabelInterval = function(date, intervalUnit) {
    ...
    case Timeline.DateTime.HOUR:
        ...
        break;
    case Timeline.DateTime.DAY:
        text = Timeline.GregorianDateLabeller.getMonthName(date.getUTCMonth(), this._locale) + " " + date.getUTCDate()+ " "+ date.getUTCFullYear();
        break;
    case Timeline.DateTime.WEEK:
        text = Timeline.GregorianDateLabeller.getMonthName(date.getUTCMonth(), this._locale) + " " + date.getUTCDate()+ " "+ date.getUTCFullYear();
        break;
    case Timeline.DateTime.MONTH:
        ...
        if (m != 0) {
            text=Timeline.GregorianDateLabeller.getMonthName(m,this._locale) + '-'+ date.getUTCFullYear();
            break;
        } // else, fall through
    case Timeline.DateTime.YEAR:
    case Timeline.DateTime.DECADE:
...
Timeline.StaticTrackBasedLayout.prototype._layout = function() {
	...
	// This method creates tracks in the band based on the start and end pixel values for an event
    var layoutEvent = function(evt) {
		...
		var streamIndex = 0;
		if(evt._isInGroup){
			layout._tracks[evt.getID()] = layout._tracks[evt._groupHead.getID()];
			//Tolven.Util.log(layout._tracks[evt.getID()] + "  " +evt.getText() );
			return;
		}
		else{
			//Tolven.Util.log('no');
			...
			// if streamIndex > streams.length it means that there is an event to be painted within the existing track
			// hence we create a new track for that event
...
Timeline.DurationEventPainter = function(params) {
    ...
    this._lableLayer = null;
    this._menuActions = params.menuActions;
    this._color = params.color;
    this._top = 0;
    this._height = 0;
...
Timeline.DurationEventPainter.prototype.setHeight=function(height){
	this._height = getPixelValue(height);
	if(this._highlightLayer)
		this._highlightLayer.style.height=height;
	if(this._eventLayer)
		this._eventLayer.style.height=height;
	if(this._height > 0){
		this._lableLayer.style.display='block';
	}
	else{
	   if(this._lableLayer && this._lableLayer != null)
		this._lableLayer.style.display='none';
	}
};

Timeline.DurationEventPainter.prototype.setTop=function(top){
	this._top = getPixelValue(top);
	if(this._highlightLayer)
	this._highlightLayer.style.top=top;
	if(this._eventLayer)
	this._eventLayer.style.top=top;
	if(this._lableLayer)
	this._lableLayer.style.top = this._top+'px';
};
...
Timeline.DurationEventPainter.prototype.getIndex=function(){
    return this._index;
};
...
Timeline.DurationEventPainter.prototype.paint = function() {
    ...
    this._highlightLayer=this._band.createLayerDivNew(0,this._label);
    this._highlightLayer.setAttribute("name", "event-highlights");
    this._highlightLayer.style.height="150px";
	if(this._color.length == 6 && this._color.charAt(0)!= '#'){
		this._color = '#'+this._color;
		this._highlightLayer.style.backgroundColor = this._color;
	}
	else
    this._highlightLayer.style.backgroundColor = this._color;
    ...
    this._eventLayer=this._band.createLayerDivNew(110,this._label);
    this._eventLayer.setAttribute("name", "events");
    this._eventLayer.style.display = "none";
    if(!this._lableLayer){
	this._lableLayer = createElement('div');
	var tempStr = "";
	if(this._menuActions.length>0){
		for(j=0;j<this._menuActions.length;j++){
		if(this._menuActions[j].hasItems=='false')
			continue;

		var actionPath = this._menuActions[j].actionPath;
		tempStr = "<li id='"+actionPath+"_showDrpDwn'> <a href=\"javascript:showActionOptions('"+actionPath+"_dropdown_loc','"+actionPath+"_drpDwn')\">"+this._menuActions[j].actionName+"</a>";
		tempStr += "<s id='"+actionPath+"_dropdown_loc' style='height:4px;position:absolute;'/>";
		tempStr +="<div id='"+actionPath+"_drpDwn' class='drpDwn' style='display:none;'></div></li>";				  
		}
	}
	this._lableLayer.innerHTML = this._label + tempStr; 
	this._lableLayer.className = "timeline-band-label";
	this._timeline._containerDiv.appendChild(this._lableLayer);
     }
    ...
    //if (this._timeline.isHorizontal()) {
        ...
        var createInstantDiv = function(evt, startPixel, endPixel, streamOffset, highlightIndex, highlightOffset, highlightWidth) {
            ...
            var length = 16;
            if (showText) {
              //if(eventTheme.label.width < (evt.getText().length*7))
				//eventTheme.label.width = (evt.getText().length*7); 
                //div.style.width = (evt.getText().length*8) + "px";
                div.style.color = foreground != null ? foreground : eventTheme.label.outsideColor;
				var tempTxtNode = null;
				if(evt._isInGroup ){
					if(evt._showGroupHeader) 
						tempTxtNode = doc.createTextNode(replaceSpecialChars(evt.getText())) ;				
					else
						tempTxtNode = doc.createTextNode("  ") ;				
				}
				else
					tempTxtNode = doc.createTextNode(replaceSpecialChars(evt.getText())) ;					
				div.appendChild(tempTxtNode);	
				div.innerHTML += "<br/>";
                appendIcon(evt, div);
...
//					p._layout.getTrack(evt) * (trackHeight + trackGap));
		var streamOffset = (trackOffset + 
					evt._groupId * (trackHeight + trackGap));
		if (evt.isInstant()) {
...
Timeline.DefaultEventSource.prototype.loadJSON = function(data, url,eventSource) {
    var base = this._getBaseURL(url);
    var added = false;  
    if (data && data.events){
...
//	Grouping events based on thier type and name
	for(var i=0;i<this._events.getCount();i++){
		var _event = this._events.elementAt(i);
		var groupHead = null;
		if(_event._isInGroup == true || _event._isInGroup == 'true')
			continue;	
		var group = this._eventGroups.get(_event.getText());
		if(group == null){
			group = new Array();
			_event._isInGroup = true;
			_event._groupId = this._eventGroups._keys.length;
			//Tolven.Util.log("loadJSON "+event._groupId);
			group[0] = _event;
			this._eventGroups.put(_event.getText(),group);
			_event._group = group;
			_event._showGroupHeader = true;
			_event.groupHead = _event;
			groupHead = _event;
		}
		else{
			groupHead = _event;
		}
		for(var j=i+1;j<this._events.getCount();j++){
			var matchEvent = this._events.elementAt(j);
			if(_event.getText() == matchEvent.getText()){
				var path1 = _event._obj.path.substring(0,_event._obj.path.lastIndexOf('-'));
				var path2 = matchEvent._obj.path.substring(0,matchEvent._obj.path.lastIndexOf('-'));
				if(path1 == path2){
					matchEvent._isInGroup = true;
					matchEvent._groupId = _event._groupId;
					//Tolven.Util.log(matchEvent._groupId);
					var groupHeadStart = eventSource._layout._ether.dateToPixelOffset(groupHead._start);					
					var currEventStart = eventSource._layout._ether.dateToPixelOffset(matchEvent._start);
					var lastHeaderOffset = groupHeadStart - currEventStart;
					if(lastHeaderOffset <0)
						lastHeaderOffset = -(lastHeaderOffset);
					// display same header for the events with 2 units range and not on the same day
					// 10 should be good enough for events on same day
					if(lastHeaderOffset > (matchEvent.getText().length*8)){
						groupHead = matchEvent;
						matchEvent._showGroupHeader = true;
					}
					else
						matchEvent._showGroupHeader = false;
					group[group.length] = matchEvent;
					matchEvent._group = group;
					matchEvent._groupHead = groupHead;
				}					
			}
		}
	}
...
Timeline.DefaultEventSource.Event = function(
	...
	// params for grouping events
	this._groupId = null;
	this._isInGroup = null;
	this._groupTop = null;
	this._showGroupHeader = false;
...
Timeline.DefaultEventSource.Event.prototype = {
    ...
    if(this._obj.path){
	var divMsgContainer=doc.createElement("div");
	 serialNo++;	 
	 var ajax = new Ajax.Request(
		 contextPath + '/ajax/bubbleDispatch.jsf',
		 {   method: 'get',
              parameters: 'element='+this._obj.path+
			             '&serialNo='+serialNo +
			             '&accountUserId='+accountUserId,
		  onComplete: function(req) {divMsgContainer.innerHTML=req.responseText; elmt.appendChild(divMsgContainer);} });		
     }
     else{
...
Timeline.ClassicTheme._Impl = function() {
    ...
    this.event = {
        ...
        label: {
            insideColor:    "white",
            outsideColor:"black" ,width:200  //to display the full text of the event
        },
        highlightColors: [
            "#FFFF00",
            "#FFC000",
...
Timeline.MirrorBand=function(timeline){
	this._timeline = timeline;
	this._containerDiv = createElement('div');
	this._events = new Map;
	this._timeline._scrollLabelsDiv = this._containerDiv;
	this._containerDiv.style.width=this._timeline._containerDiv.offsetWidth+'px';
	this._containerDiv.className = "timelineBoundLables";
	this._containerDiv.style.height = "30px";
	this._containerDiv.innerHTML += "<table><tr><td>"+this._timeline.minDate.getFullYear()+"</td><td style='text-align:right'>"+this._timeline.maxDate.getFullYear()+"</td></tr></table>";	
	
}
Timeline.MirrorBand.prototype.paint=function(eventPainter){ 
	...
	for(var i=0;i<_painterEvents.getCount();i++){
		...
		_evt._left = _offsetFrmMinDate+'%';
		this.addTickMark(_evt);
		_mirrorEvents.push(_evt);
	}
	this._events.put(eventPainter._path,_mirrorEvents);
}
Timeline.MirrorBand.prototype.addTickMark = function(_mirrorEvent){
	_mirrorEvent._tickMark.style.left = _mirrorEvent._left;
	_mirrorEvent._tickMark.style.top = 	((_mirrorEvent._event._groupId+1) * (_mirrorEvent._eventPainter._index+1) *5)+"px";
	if((getPixelValue(_mirrorEvent._tickMark.style.top)+10) > getPixelValue(this._containerDiv.style.height)){
		this._containerDiv.style.height = getPixelValue(_mirrorEvent._tickMark.style.top) + 15 + 'px';
	}
	this._containerDiv.appendChild(_mirrorEvent._tickMark);
}
Timeline.MirrorEvent = function(evnt,eventPainter,timeline,leftPos){
	this._event = evnt;
	this._eventPainter = eventPainter;
	this._timeline = timeline;
	this._left = 0;
