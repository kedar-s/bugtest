...
import org.tolven.app.filter.Filter;
import org.tolven.app.filter.FilterLexer;
import org.tolven.app.filter.FilterParser;
...
/*            selectString.append( "DISTINCT md.id as pk" );
		...
		if(dq.getFilteredList()!= null && dq.getFilteredList().size() > 0){
			whereString.append(	" AND md.id in (:filteredList) ");
			params.put("filteredList", dq.getFilteredList());			
		}
		if(!countOnly) {
	...
	public DataQueryResults setupQuery(String menupath, AccountUser accountUser) {
		...
		if (menupath.startsWith(":")) {
			path = accountUser.getAccount().getAccountType().getKnownType() + menupath;
		} else {
			path = menupath;
		}
		...
		if (menupath.startsWith("global:")) {
	        ...
	        ms = findAccountMenuStructure(accountUser.getAccount(), mp);
		}
	...
	public void addExtendedFields(DataQueryResults dq) throws Exception{
		try{
	 ...
	 * @throws Exception 
	 */
	public void addParentFields(DataQueryResults dq) throws Exception {
	 ...
	 * @throws Exception 
	 */
	private void updateParentFields(DataQueryResults newDq, List<DataField> newFields, List<Object> newRows, boolean fieldFlag, String externalPrefix, boolean addIdFlag, boolean filterParent) throws Exception {
