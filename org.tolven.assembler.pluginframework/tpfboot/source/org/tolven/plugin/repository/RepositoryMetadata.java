...
import org.tolven.plugin.repository.bean.IncludeDetail;
import org.tolven.plugin.repository.bean.IncludesDetail;
...
public class RepositoryMetadata {
    ...
    public static final String INTEGER_MAX_STRING = String.valueOf(Integer.MAX_VALUE);
    ...
    public static Plugins expandIncludedPlugins(Plugins plugins, File includesDir) {
        IncludesDetail includes = plugins.getIncludes();
        if (includes == null || includes.getInclude().isEmpty()) {
            return plugins;
        }
        Plugins expandedPlugins = plugins;
        for (IncludeDetail include : includes.getInclude()) {
            String pluginsRef = include.getPluginsRef();
            File newIncludesDir = null;
            URL pluginsURL = null;
            try {
                if (pluginsRef.startsWith("http")) {
                    pluginsURL = new URL(pluginsRef);
                    newIncludesDir = includesDir;
                } else if (pluginsRef.startsWith("file")) {
                    pluginsURL = new URL(pluginsRef);
                    File file = new File(pluginsURL.getFile());
                    newIncludesDir = file.getParentFile();
                } else {
                    File file = new File(includesDir, pluginsRef);
                    newIncludesDir = file.getParentFile();
                    pluginsURL = file.toURI().toURL();
                }
            } catch (Exception ex) {
                throw new RuntimeException("Could not convert to URL: " + pluginsRef, ex);
            }
            Plugins pluginsToInclude = getPlugins(pluginsURL);
            pluginsToInclude = expandIncludedPlugins(pluginsToInclude, newIncludesDir);
            if ("overlay".equals(include.getIncludeType())) {
                merge(pluginsToInclude, expandedPlugins);
            } else {
                merge(expandedPlugins, pluginsToInclude);
                expandedPlugins = pluginsToInclude;
            }
        }
        expandedPlugins.setIncludes(null);
        return expandedPlugins;
    }

    private static void format(PluginPropertyDetail property, Comparator<PluginPropertyDetail> propertyDetailComparator) {
        List<PluginPropertyDetail> childProperties = property.getProperty();
        if (childProperties.isEmpty()) {
            return;
        }
        Collections.sort(childProperties, propertyDetailComparator);
        for (PluginPropertyDetail childProperty : property.getProperty()) {
            format(childProperty, propertyDetailComparator);
        }
    }
    ...
    public static String format(Plugins plugins) {
        ...
        for (PluginDetail pluginDetail : pluginDetails) {
            for (PluginPropertyDetail property : pluginDetail.getProperty()) {
                format(property, propertyDetailComparator);
            }
        }
        List<PluginPropertyDetail> properties = plugins.getProperty();
        Collections.sort(properties, propertyDetailComparator);
        for (PluginPropertyDetail property : plugins.getProperty()) {
            format(property, propertyDetailComparator);
        }
    ...
    public static Plugins getPlugins(File pluginsFile) {
        try {
            String pluginsXML = FileUtils.readFileToString(pluginsFile);
            return getPlugins(pluginsXML, pluginsFile.getParentFile());
        } catch (IOException ex) {
    ...
    public static Plugins getPlugins(InputStream xmlStream) {
        try {
            Unmarshaller u = getJaxbContext().createUnmarshaller();
            return (Plugins) u.unmarshal(xmlStream);
        } catch (JAXBException ex) {
            throw new RuntimeException("Could not load plugins inputstream", ex);
        }
    }
    ...
    public static Plugins getPlugins(String pluginsXML) {
        return getPlugins(pluginsXML, null);
    }
     ...
     * @return
     */
    private static Plugins getPlugins(String pluginsXML, File includesDir) {
        Plugins plugins = null;
        try {
            StringReader reader = new StringReader(pluginsXML);
            Unmarshaller u = getJaxbContext().createUnmarshaller();
            plugins = (Plugins) u.unmarshal(reader);
        } catch (JAXBException ex) {
            throw new RuntimeException("Could not read plugins from: " + pluginsXML, ex);
        }
        return expandIncludedPlugins(plugins, includesDir);
    }

    private static Plugins getPlugins(URL pluginsXMLURL) {
    ...
    private static PluginPropertyDetail getProperty(String name, List<PluginPropertyDetail> properties) {
    ...
    public static void main(String[] args) {
        ...
        cmdLineOptions.addOption(new Option("relativePath", "relativePath", true, "Uses plugin paths relative to metadata plugins.xml directory (plugins is the default)"));
        ...
        String relativePath = commandLine.getOptionValue("relativePath");
        String libraryURLString = commandLine.getOptionValue("liburl");
        String outDirname = commandLine.getOptionValue("outdir");
        new RepositoryMetadata().generate(pluginsDirname, libraryURLString, relativePath, outDirname);
    }

    private static void merge(PluginDetail srcPlugin, PluginDetail targetPlugin) {
        if (targetPlugin.getRoot() == null && srcPlugin.getRoot() != null) {
            targetPlugin.setRoot(new RootPluginDetail());
        } else if (targetPlugin.getRoot() != null && srcPlugin.getRoot() == null) {
            targetPlugin.setRoot(null);
        }
        for (PluginPropertyDetail srcProperty : srcPlugin.getProperty()) {
            PluginPropertyDetail targetProperty = getProperty(srcProperty.getName(), targetPlugin.getProperty());
            if (targetProperty == null) {
                targetPlugin.getProperty().add(srcProperty);
            } else if (srcProperty.isEnabled() != null && !srcProperty.isEnabled()) {
                targetPlugin.getProperty().remove(targetProperty);
            } else {
                merge(srcProperty, targetProperty);
            }
        }
    }

    private static void merge(PluginPropertyDetail srcProperty, PluginPropertyDetail targetProperty) {
        targetProperty.setValue(srcProperty.getValue());
        List<PluginPropertyDetail> childSrcProperties = srcProperty.getProperty();
        if (childSrcProperties.isEmpty()) {
            return;
        }
        for (PluginPropertyDetail childSrcProperty : childSrcProperties) {
            PluginPropertyDetail childTargetProperty = getProperty(childSrcProperty.getName(), targetProperty.getProperty());
            if (childTargetProperty == null) {
                targetProperty.getProperty().add(childSrcProperty);
            } else if (childSrcProperty.isEnabled() != null && !childSrcProperty.isEnabled()) {
                targetProperty.getProperty().remove(childTargetProperty);
            } else {
                merge(childSrcProperty, childTargetProperty);
            }
        }
    }

    public static void merge(Plugins srcPlugins, Plugins targetPlugins) {
        for (PluginDetail srcPlugin : srcPlugins.getPlugin()) {
            PluginDetail targetPlugin = getPluginDetail(srcPlugin.getId(), targetPlugins);
            if (targetPlugin == null) {
                targetPlugins.getPlugin().add(srcPlugin);
            } else if (srcPlugin.isEnabled() != null && !srcPlugin.isEnabled()) {
                targetPlugins.getPlugin().remove(targetPlugin);
            } else {
                merge(srcPlugin, targetPlugin);
            }
        }
        for (PluginPropertyDetail srcProperty : srcPlugins.getProperty()) {
            PluginPropertyDetail targetProperty = getProperty(srcProperty.getName(), targetPlugins.getProperty());
            if (targetProperty == null) {
                targetPlugins.getProperty().add(srcProperty);
            } else if (srcProperty.isEnabled() != null && !srcProperty.isEnabled()) {
                targetPlugins.getProperty().remove(targetProperty);
            } else {
                merge(srcProperty, targetProperty);
            }
        }
    ...
    public void generate(String pluginsDirname, String libraryURLString, String relativePath, String outDirname) {
        logger.debug("*** start ***");
        ...
        URL libraryPluginsURL = null;
        try {
            if (libraryURLString == null) {
                libraryPluginsURL = sourcePluginsDir.getParentFile().toURI().toURL();
            } else {
                libraryPluginsURL = new URL(libraryURLString);
            }
        } catch (MalformedURLException ex) {
            throw new RuntimeException("Could not convert to URL", ex);
        }
        if (outDirname == null) {
            setXmlOutputDir(sourcePluginsDir.getParentFile());
        } else {
            setXmlOutputDir(new File(outDirname));
        }
        setObjectFactory(new ObjectFactory());
        URL sourceLibraryPluginsURL = null;
        try {
        ...
        Plugins libraryPlugins = getPlugins(plugins, false, sourceLibraryPluginsURL.toExternalForm(), libraryPluginsURL.toExternalForm());
        if (relativePath != null) {
            useRelativePaths(libraryPlugins, relativePath);
        }
    ...
    private void useRelativePaths(Plugins plugins, String relativePath) {
        String uriPath = relativePath + "/";
        for (PluginDetail plugin : plugins.getPlugin()) {
            for (PluginVersionDetail pluginVersion : plugin.getVersion()) {
                File zipFile = new File(pluginVersion.getUri());
                pluginVersion.setUri(uriPath + zipFile.getName());
            }
        }
    }
