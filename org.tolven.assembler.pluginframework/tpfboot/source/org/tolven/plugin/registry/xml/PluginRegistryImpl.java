...
import org.tolven.plugin.registry.xml.IntegrityChecker.ReportItemImpl;
...
public class PluginRegistryImpl implements PluginRegistry {
    static final String PACKAGE_NAME = "org.tolven.plugin.registry.xml"; //$NON-NLS-1$
    ...
    public void configure(final ExtendedProperties config) {
        ...
        log.debug("configured, stopOnError=" + stopOnError //$NON-NLS-1$
    ...
    public Map<String, Identity> register(final URL[] manifests)
        ...
        log.debug("plug-in and fragment descriptors registered - " //$NON-NLS-1$
                + result.size());
        dump();
        if (result.isEmpty()) {
            return result;
    ...
    private PluginFragment registerFragment(final ModelPluginFragment model,
        ...
        try {
            ...
            if (!isRegistered) {
                log.debug("no matching plug-ins found for fragment " //$NON-NLS-1$
                        + result.getUniqueId());
                registrationReport.add(new ReportItemImpl(
    ...
    public Collection<String> unregister(final String[] ids) {
        ...
        registrationReport.add(new ReportItemImpl(
                ...
                Integer.valueOf(registeredFragments.size())));
        log.debug("plug-in and fragment descriptors unregistered - " //$NON-NLS-1$
                + result.size());
        dump();
        if (result.isEmpty()) {
            return result;
    ...
    public ExtensionPoint getExtensionPoint(final String pluginId,
        ...
        for (ExtensionPoint point : descriptor.getExtensionPoints()) {
            if (point.getId().equals(pointId)) {
                ...
                log.debug("extension point " + point.getUniqueId() //$NON-NLS-1$
    ...
    public void unregisterListener(final RegistryChangeListener listener) {
        if (!listeners.remove(listener)) {
            log.debug("unknown listener " + listener); //$NON-NLS-1$
        }
    }
    ...
    private static class RegistryChangeDataImpl
    ...
    private static class ManifestInfoImpl implements ManifestInfo {
