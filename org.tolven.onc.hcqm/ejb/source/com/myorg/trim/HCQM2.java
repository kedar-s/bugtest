...
public class HCQM2 extends ComputeBase {
	 ...
	 //admitOrderDateTime
		...
		if (trim != null) {
			...
			for (ActRelationship actRelationship : relList) {
				...
				/************* IMPLEMENT THE LOGIC FOR STK1.1,STK1.2,STK1.3,STK2.1,STK2.2,STK2.3*******START*********/
				//Set the STK 1 Measure
				 
				if (actRelationship.getName().equalsIgnoreCase("stk2")) {
					...
					for (ObservationValueSlot observationValueSlot : observationValueSlotList) {
						//All Patients admitted to hospital from STK (dischargestatus code 09)
				...
				if (actRelationship.getName().equalsIgnoreCase("stk3")) {
					List<ObservationValueSlot> observationValueSlotList = actRelationship.getAct().getObservation().getValues();
					...
					for (ObservationValueSlot observationValueSlot : observationValueSlotList) {
							...
							//check if the length of stay in STK is less than 2 days or more than 120 days 
							if( lengthofStay > 120) {
								observationValueSlot.getST().setValue("X");
							} 	
					} 
				...
				if (actRelationship.getName().equalsIgnoreCase("stk4")) {
					List<ObservationValueSlot> observationValueSlotList = actRelationship.getAct().getObservation().getValues();
				...
				if (actRelationship.getName().equalsIgnoreCase("stk5")) {
					List<ObservationValueSlot> observationValueSlotList = actRelationship.getAct().getObservation().getValues();
				...
				if (actRelationship.getName().equalsIgnoreCase("stk6")) {
					List<ObservationValueSlot> observationValueSlotList = actRelationship.getAct().getObservation().getValues();
				...
				if (actRelationship.getName().equalsIgnoreCase("stk8")) {
					List<ObservationValueSlot> observationValueSlotList = actRelationship.getAct().getObservation().getValues();
				...
				if (actRelationship.getName().equalsIgnoreCase("stk10")) {
					List<ObservationValueSlot> observationValueSlotList = actRelationship.getAct().getObservation().getValues();
				...
				/*************  THE LOGIC FOR STK1.1,STK1.2,STK1.3,STK1.4,STK1.5,STK1.6********ENDS********/
			}
		}
	...
	public long getLengthofStay(Trim trim) {
		...
		diffMinutes = getDateDifference(edAdmissionDate, edDischDate, HCQM2.MINUTES);
	...
	public long getWait2(Trim trim) {
		...
		Date admOrderDate = convertStringToDate(admOrderDateTime, "yyyyMMddHHmm");
		if(admOrderDate ==null) {
			admOrderDate = convertStringToDate(admOrderDateTime, "yyyyMMddHHmmssZ");
		}
		...
		diffMinutes = getDateDifference(admOrderDate,edDiscDate, HCQM2.MINUTES);
	...
	public long getPatientAge(Trim trim) {
		...
		patientAge = getDateDifference(patientDobDate,admOrderDate, HCQM2.YEARS);
        System.out.println("=====================patientAge================="+patientAge);
...
*/
	...
	public long convertStringToDateDifference(String dateBegin,String dateEnd,String option,String format) {
		...
		try {
			...
			if(option.equals(HCQM2.SECONDS)){
				difference = edDiff / (1000);
			
			}
			
			if(option.equals(HCQM2.MINUTES)){
				difference = edDiff/(1000*60);
			}
			
			if(option.equals(HCQM2.HOURS)){
				difference   = edDiff/(60*60*1000);
			}
			
			if(option.equals(HCQM2.DAYS)){
				difference    = (edDiff/(24 * 60 * 60 * 1000));
			}
			
			if(option.equals(HCQM2.YEARS)){
