...
public class LoadRESTfulClient extends RESTfulClient {
    ...
    protected int getIterationLimit() {
        if (limit == null) {
            try {
                ...
                String limitString = System.getProperty(getIterationLimitProperty());
                if (limitString == null) {
                    limitString = System.getProperty(UPLOAD_LIMIT);
                }
                if (limitString != null) {
    ...
    protected String getIterationLimitProperty() {
    ...
    public void createTrimHeader(String xml) {
        ...
        ClientResponse response = webResource.cookie(getTokenCookie()).type(MediaType.APPLICATION_XML).entity(xml).post(ClientResponse.class);
